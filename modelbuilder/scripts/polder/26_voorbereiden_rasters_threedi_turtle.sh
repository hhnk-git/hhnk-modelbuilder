...
ogr2ogr -overwrite -sql "SELECT ST_SetSRID(ST_CollectionExtract(geom,3),28992) as geom, id FROM deelgebied.channelsurfacedem" -f "ESRI Shapefile" /code/tmp/rasters/tmp/channelsurface.shp PG:"host=db user=postgres dbname=datachecker password=postgres port=5432"
