...
ogr2ogr -overwrite -sql "SELECT polder_id, name, polder_type, ST_SetSRID(geom,28992) as geom FROM deelgebied.polder" -f "ESRI Shapefile" /code/tmp/rasters/tmp/polder.shp PG:"host=db user=postgres dbname=datachecker password=postgres port=5432"
