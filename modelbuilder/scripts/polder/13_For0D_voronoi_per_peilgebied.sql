...
--Cleanup geometries
UPDATE deelgebied.fixeddrainagelevelarea SET geom = ST_CollectionExtract(ST_MakeValid(geom),3);
...
CREATE TABLE tmp.voronoi_peilgebieden AS(
	...
	FROM deelgebied.fixeddrainagelevelarea a, deelgebied.polder b
    WHERE ST_Intersects(b.geom,a.geom)
);
DROP INDEX IF EXISTS tmp.vrn_peilgebied;
CREATE INDEX vrn_peilgebied ON tmp.voronoi_peilgebieden USING gist(area_geom);
...
CREATE TABLE tmp.voronoi_imaginaire_punten AS(
	WITH envelope AS(
	SELECT 
		ST_Envelope(ST_UNION(ST_MakeValid(area_geom))) as env_geom,	--bounding box om deelgebied
		ST_Scale(ST_Envelope(ST_UNION(ST_MakeValid(area_geom))),5,5) as env_geom_scale, --schaal bounding box 5x
		ST_Centroid(ST_Envelope(ST_UNION(ST_MakeValid(area_geom)))) as env_geom_centroid, --middelpunt bounding box (voor translate operatie)
	 ...
	 FROM tmp.voronoi_peilgebieden
	)
	SELECT DISTINCT((ST_DumpPoints(ST_Translate(env_geom_scale,ST_X(env_geom_centroid)-ST_X(env_geom_scale_centroid),ST_Y(env_geom_centroid)-ST_Y(env_geom_scale_centroid)))).geom) as img_point
	FROM envelope
);
...
CREATE TABLE tmp.voronoi_puntdata AS(
	...
	FROM public.v2_connection_nodes a  --inputtabel
	LEFT JOIN tmp.voronoi_peilgebieden b
	ON ST_Contains(b.area_geom,a.the_geom)
);
...
DELETE FROM tmp.voronoi_puntdata 
WHERE point_id IN (
...
DELETE FROM tmp.voronoi_puntdata 
WHERE point_id IN (SELECT connection_node_id FROM v2_1d_boundary_conditions)
;
...
CREATE TABLE tmp.voronoi_input AS(
	SELECT 
		...
		ST_COLLECTIONEXTRACT(ST_COLLECT(
			ST_UNION(point_geom),
				(SELECT ST_UNION(img_point) FROM tmp.voronoi_imaginaire_punten)
			
			),1) point_geom
	FROM (
		 SELECT a.point_geom, b.*
		 FROM tmp.voronoi_puntdata as a, tmp.voronoi_peilgebieden as b
		 WHERE ST_Contains(b.area_geom,a.point_geom)
	) a
	GROUP BY area_id
);
...
CREATE TABLE tmp.voronoi_edges AS(
	...
	SELECT 
		...
		UNNEST(ARRAY[
			ST_SETSRID(ST_MakeLine(p1,p2),28992) ,
			ST_SETSRID(ST_MakeLine(p2,p3),28992) ,
			ST_SETSRID(ST_MakeLine(p3,p1),28992)]) Edge,
	...
	FROM    (
		...
		FROM    (
			SELECT area_id, (gd).Path id, ST_ExteriorRing((gd).Geom) g -- ID andmake triangle a linestring
			FROM (
				SELECT area_id, (ST_Dump(ST_DelaunayTriangles(point_geom))) gd 
				FROM tmp.voronoi_input
			) a 
		)b
	) c
);
DROP INDEX IF EXISTS tmp.vrn_edges;
CREATE INDEX vrn_edges ON tmp.voronoi_edges USING gist(edge);
...
CREATE TABLE tmp.voronoi_inner_lines AS(
		...
		x.ct,
		CASE 
		WHEN ST_IsEmpty(y.ct) THEN
		    CASE WHEN ST_Within(
			x.ct,
			(SELECT hull FROM tmp.voronoi_convex WHERE area_id=x.area_id)) THEN -- Don't draw lines back towards the original set
			-- Project line out twice the distance from convex hull
			ST_MakePoint(ST_X(x.ct) + ((ST_X(ST_Centroid(x.edge)) - ST_X(x.ct)) * 2),ST_Y(x.ct) + ((ST_Y(ST_Centroid(x.edge)) - ST_Y(x.ct)) * 2))
		    END
		ELSE 
			y.ct --ST_SETSRID(y.ct,28992)
		END
		))) v
	    FROM tmp.voronoi_edges x 
		LEFT OUTER JOIN -- Self Join based on edges
		tmp.voronoi_edges y ON x.id <> y.id AND ST_Equals(x.edge,y.edge)
	    GROUP BY x.area_id
);
...
CREATE TABLE tmp.voronoi_lines AS(
	...
	FROM tmp.voronoi_inner_lines
	WHERE ST_GeometryType(ST_ConvexHull(v)) = 'ST_Polygon'
);
...
CREATE TABLE tmp.voronoi_unclipped AS(
	...
	a.area_id, 
	a.voronoi_unclipped
	FROM tmp.voronoi_polygon a
	LEFT JOIN tmp.voronoi_puntdata b
	ON a.area_id = b.area_id AND ST_Contains(a.voronoi_unclipped,b.point_geom)
);
...
CREATE TABLE deelgebied.voronoi_output AS(
	...
	FROM tmp.voronoi_clip
	WHERE NOT ST_IsEmpty(vrn_geom)
	GROUP BY point_id
	);
...
CREATE TABLE tmp.voronoi_invalid_peilgebied AS(
	SELECT * 
	FROM tmp.voronoi_peilgebieden 
	WHERE NOT ST_IsValid(area_geom) 
		OR area_id IN 
			(
			SELECT DISTINCT(area_id) FROM tmp.voronoi_invalid_polygon
			)
);
...
CREATE TABLE tmp.voronoi_cover AS(
	...
	FROM deelgebied.voronoi_output
	GROUP BY area_id
	)
	SELECT
	a.id,
	...
	FROM deelgebied.fixeddrainagelevelarea a
	LEFT JOIN opsomming b
	ON a.id = b.area_id
);

ALTER TABLE tmp.voronoi_cover ADD COLUMN prec double precision DEFAULT 0;
UPDATE tmp.voronoi_cover SET prec = vrn_sum/org_area WHERE org_area != 0 AND vrn_sum IS NOT NULL AND org_area IS NOT NULL;
...
WITH lijst AS (
 SELECT point_id as nodeid, area_id as vlakid, vrn_area as opppernode
 FROM deelgebied.voronoi_output
 )
