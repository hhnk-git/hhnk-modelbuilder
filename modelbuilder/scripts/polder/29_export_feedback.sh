...
ogr2ogr -overwrite -sql "SELECT * FROM deelgebied.impervious_surface_simple" -nln impervious_surface_simple -f "ESRI Shapefile" /code/data/output/models/feedback/impervious_surface_simple.shp PG:"host=db user=postgres dbname=datachecker port=5432 password=postgres" -nlt MultiPoint -a_srs EPSG:28992

# Polder polygon
ogr2ogr -overwrite -sql "SELECT * FROM deelgebied.polder" -nln polder_polygon -f "ESRI Shapefile" /code/data/output/plugin/polder_polygon.shp PG:"host=db user=postgres dbname=datachecker port=5432 password=postgres" -a_srs EPSG:28992
...
ogr2ogr -clipsrc /code/data/output/plugin/polder_polygon.shp -overwrite -sql "SELECT * FROM deelgebied.fixeddrainagelevelarea" -nln peilgebieden -f "ESRI Shapefile" /code/data/output/plugin/peilgebieden.shp PG:"host=db user=postgres dbname=datachecker port=5432 password=postgres" -a_srs EPSG:28992
