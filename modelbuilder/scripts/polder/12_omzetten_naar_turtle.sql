...
WITH channel_xsloc_join AS(
	SELECT a.id as channel_id, connection_node_start_id, connection_node_end_id, b.id as xs_loc_id, b.reference_level, ST_LineLocatePoint(a.the_geom, b.the_geom) as position_on_channel 
	FROM v2_channel a, v2_cross_section_location b
	WHERE a.id = b.channel_id
	)
, ascending_position AS(
	SELECT channel_id, xs_loc_id, connection_node_start_id as connection_node_id, reference_level 
	FROM channel_xsloc_join
	ORDER BY channel_id, position_on_channel ASC
	)
, descending_position AS(
	SELECT channel_id, xs_loc_id, connection_node_end_id as connection_node_id, reference_level 
	FROM channel_xsloc_join
...
SELECT connection_node_id, min(reference_level) as min_reference_level 
FROM connection_nodes_channel_reference_level 
GROUP BY connection_node_id 
ORDER BY connection_node_id
);

DROP TABLE IF EXISTS tmp.bridge_crest_level;
CREATE TABLE tmp.bridge_crest_level AS(
SELECT a.*, LEAST(b.min_reference_level, c.min_reference_level) as lowest_bed_level 
FROM v2_orifice a, tmp.connection_node_min_reference_level b, tmp.connection_node_min_reference_level c 
...
SELECT a.id, a.crest_level, a.code, a.cross_section_definition_id, b.bottom_level, c.height, greatest(1,b.bottom_level-a.crest_level) as new_height 
FROM v2_orifice a, deelgebied.bridge b, v2_cross_section_definition c
