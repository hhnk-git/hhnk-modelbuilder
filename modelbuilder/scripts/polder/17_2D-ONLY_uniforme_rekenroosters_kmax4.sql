...
AS
    ...
    IF EXISTS k;
        ...
        create table tmp.gridspace_kmax as
        select
               nextval('k')::integer as k
             , NULL::integer         as grid_size
        from
        ...
        with parameters as
             (
                    select
                           20 as grid_space
                         , 4  as kmax
             ) --> standaard bwn = 20x20, 40x40, 80x80, 160x160m cellen.
        update
               tmp.gridspace_kmax a
        set    grid_size = b.grid_space
        from
               parameters b
        where
               a.k <= b.kmax
        ...
        */
        ...
        SELECT
                 split_part(split_part(ST_Extent(geom)::varchar, '(', 2),' ',1)::numeric                                 as x0
        ...
        FROM
                 ...
                 as foo -- REPLACE WITH YOUR EXTENT SHAPEFILE
        GROUP BY
                 polder_id
               , geom
        ;
        ...
        with grid_size       as
             (
                    ...
                    where
                           k=4
             ) -- kmax+1 <-- als wij kmax=4 dan hebben een tmp.k5 rekenrooster nodig voor het querien
        SELECT
               ST_SetSRID((ST_Dump(ST_Collect(cells.geom))).geom,28992) as geom
        FROM
               ST_CreateFishnet(
               (
                      SELECT
                             ceil((ym-y0)/size)::integer
                      FROM
                             grid_size
                           , tmp.box
               )
        ...
        IF EXISTS id;
            ...
            DROP TABLE IF EXISTS tmp.k5
            ; -- CHANGE tmp.k to K YOU WANT
            CREATE TABLE tmp.k5 AS
            SELECT
                   b.geom
                 , nextval('id') as pk -- CHANGE tmp.k to K YOU WANT
            FROM
                   tmp.box as a
                 , tmp.net as b
            WHERE
            ...
            SELECT
                   ...
                   , geom
            FROM
                     tmp.outerring -- REPLACE WITH YOUR EXTENT SHAPEFILE
            GROUP BY
                     id
                   , geom
            ;
            ...
            IF EXISTS id;
                CREATE SEQUENCE id RESTART WITH 1;
                DROP TABLE IF EXISTS tmp.k4
                ; -- CHANGE tmp.k to K YOU WANT
                CREATE TABLE tmp.k4 AS
                SELECT
                       b.geom
                     , nextval('id') as pk -- CHANGE tmp.k to K YOU WANT
                FROM
                ...
                IF EXISTS id;
                    CREATE SEQUENCE id RESTART WITH 1;
                    DROP TABLE IF EXISTS tmp.k3
                    ; -- CHANGE tmp.k to K YOU WANT
                    CREATE TABLE tmp.k3 AS
                    SELECT
                           b.geom
                         , nextval('id') as pk -- CHANGE tmp.k to K YOU WANT
                    FROM
                    ...
                    IF EXISTS id;
                        CREATE SEQUENCE id RESTART WITH 1;
                        DROP TABLE IF EXISTS tmp.k2
                        ; -- CHANGE tmp.k to K YOU WANT
                        CREATE TABLE tmp.k2 AS
                        SELECT
                               b.geom
                             , nextval('id') as pk -- CHANGE tmp.k to K YOU WANT
                        FROM
                        ...
                        IF EXISTS id;
                            CREATE SEQUENCE id RESTART WITH 1;
                            DROP TABLE IF EXISTS tmp.k1
                            ; -- CHANGE tmp.k to K YOU WANT
                            CREATE TABLE tmp.k1 AS
                            SELECT
                                   b.geom
                                 , nextval('id') as pk -- CHANGE tmp.k to K YOU WANT
                            FROM
                            ...
                            ; -- 7sec
                            DROP INDEX
                            IF EXISTS tmp.k3_index_geom;
                                CREATE INDEX k3_index_geom
                                ON
                                             tmp.k3
                                USING        gist
                                             (
                                ...
                                DROP INDEX
                                IF EXISTS tmp.k2_index_geom;
                                    CREATE INDEX k2_index_geom
                                    ON
                                                 tmp.k2
                                    USING        gist
                                                 (
                                    ...
                                    DROP INDEX
                                    IF EXISTS tmp.k1_index_geom;
                                        CREATE INDEX k1_index_geom
                                        ON
                                                     tmp.k1
                                        USING        gist
                                                     (
                                        ...
                                        CREATE INDEX deelgebied_channel_peil_geom
                                        ON
                                                     deelgebied.channel_peil
                                        USING        gist
                                                     (
                                                                  geom
                                                     )
                                        ...
                                        CREATE INDEX deelgebied_channel_peil_union_geom
                                        ON
                                                     deelgebied.channel_peil_union
                                        USING        gist
                                                     (
                                                                  geom
                                                     )
                                        ...
                                        SELECT
                                                  a.*
                                                , count(b.teller)
                                                , max(maximum_water_level) as max_peil -- HIER STARKS LEVEE HEIGHT
                                        FROM
                                                  tmp.k4 as a
                                                  LEFT JOIN
                                                            deelgebied.channel_peil_union as b
                                                            ON
                                                                      ST_Intersects(a.geom,b.geom)
                                                  LEFT JOIN
                                                            deelgebied.peilgrens_met_waterpeil as c
                                                            ON
                                        ...
                                        DROP INDEX
                                        IF EXISTS tmp.k5_index_pk;
                                            CREATE INDEX k5_index_pk
                                            ON
                                                         tmp.k5
                                            USING        btree
                                                         (
                                            ...
                                            DROP INDEX
                                            IF EXISTS tmp.k4_index_pk;
                                                CREATE INDEX k4_index_pk
                                                ON
                                                             tmp.k4
                                                USING        btree
                                                             (
                                                ...
                                                DROP INDEX
                                                IF EXISTS tmp.k3_index_pk;
                                                    CREATE INDEX k3_index_pk
                                                    ON
                                                                 tmp.k3
                                                    USING        btree
                                                                 (
                                                    ...
                                                    DROP INDEX
                                                    IF EXISTS tmp.k2_index_pk;
                                                        CREATE INDEX k2_index_pk
                                                        ON
                                                                     tmp.k2
                                                        USING        btree
                                                                     (
                                                        ...
                                                        DROP INDEX
                                                        IF EXISTS tmp.k1_index_pk;
                                                            CREATE INDEX k1_index_pk
                                                            ON
                                                                         tmp.k1
                                                            USING        btree
                                                                         (
