...
CREATE TABLE tmp.pumpstation_peil AS
             (
                       ...
                       FROM
                                 v2_pumpstation a
                                 LEFT JOIN
                                           tmp.connection_node_fdla b
                                           ON
                                                     a.connection_node_start_id = b.connection_node_id
                                 LEFT JOIN
                                           tmp.connection_node_fdla c
                                           ON
                                                     a.connection_node_end_id = c.connection_node_id
                                 LEFT JOIN
                                           deelgebied.pumpstation d
                                           ON
                                                     a.code = d.code
             )
;
...
WHERE
       ...
       (
              SELECT
                     code
              FROM
                     tmp.pumpstation_peil
              WHERE
                     schema_from_fixeddrainagelevelarea_code LIKE '%1000-01%'
                     AND
                     (
                            data_to_fixeddrainagelevelarea_code   NOT LIKE '%1000-01%'
                            OR data_to_fixeddrainagelevelarea_code IS NULL
                     )
       )
;
   ...
   , end_channel AS
     (
            ...
            FROM
                 ...
                 , end_node   as b
            WHERE
                   b.id    = a.connection_node_end_id
                   OR b.id = a.connection_node_start_id
     )
DELETE
FROM
...
UPDATE
       v2_pumpstation
SET    connection_node_end_id = NULL
     , zoom_category          = 5
WHERE
       connection_node_end_id IN
       (
              SELECT *
              FROM
                     end_node
