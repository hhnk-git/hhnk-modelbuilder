...
FROM
       deelgebied.syphon_channel_union b
;
...
CREATE TABLE deelgebied.channel_snapped_syphon_clip AS
             (
                    SELECT
                           (ST_Dump(ST_Difference(a.geom,b.geom))).geom as dumpgeom
                         , a.*
                    FROM
                           deelgebied.channel a
                         , deelgebied.syphon  b
                    WHERE
...
DROP SEQUENCE
IF EXISTS seq_channel_id;
    CREATE SEQUENCE seq_channel_id START 1;
    SELECT
           setval('seq_channel_id', max(id))
    FROM
           deelgebied.channel
    ;
    ...
    WHERE
           ...
           (
                  SELECT
                         id
                  FROM
                         deelgebied.channel_snapped_syphon_clip
           )
    ;
    ...
    WHERE
           ...
           (
                  SELECT
                         id
                  FROM
                         tmp.exact_overlapping_channel_syphon
           )
    ;
    
    UPDATE
           deelgebied.channel_snapped_syphon_clip
    SET    id   = nextval('seq_channel_id')
         , geom = dumpgeom
    ;
    ...
    --Insert parts
    INSERT INTO deelgebied.channel
    SELECT *
    FROM
           deelgebied.channel_snapped_syphon_clip
    WHERE
