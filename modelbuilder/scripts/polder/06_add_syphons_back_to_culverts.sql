...
INSERT INTO deelgebied.culvert
SELECT *
FROM
       deelgebied.syphon
;
...
DROP SEQUENCE
IF EXISTS seq_culvert_snapped_id;
    CREATE SEQUENCE seq_culvert_snapped_id START 1;
    SELECT
           setval('seq_culvert_snapped_id', max(id))
    FROM
           deelgebied.culvert_snapped
    ;
               ...
               , fracties_van_tot_opgeschoond AS
                 (
                        ...
                        FROM
                               fracties_van_tot
                        WHERE
                               fractie_van     IS NOT NULL
                               AND fractie_tot IS NOT NULL
                 )
            SELECT
                   ARRAY[row_number() over()]                       AS path
                 , ST_LineSubString(geom, fractie_van, fractie_tot) AS geom
            FROM
                   fracties_van_tot_opgeschoond LOOP RETURN NEXT i
            ;
    ...
    AS
        ...
        BEGIN
            ...
            WITH ser AS
                 (
                        select
                               generate_series(0::integer, ceil(1/step)::integer, step) AS ies
                 )
            SELECT
                   array_agg(ser.ies)
            FROM
            ...
            FOR i IN
            SELECT
                   (ST_LineSubstrings(inputlijn, cut_fractions)).* LOOP RETURN NEXT i
            ;
    ...
    CREATE TABLE tmp.split_channels AS
                 (
                      ...
                      SELECT
                               a.id                                          as channel_cut_circular_id
                             , array_agg(ST_LineLocatePoint(a.geom, b.geom)) as fraction_arr
                      FROM
                               deelgebied.channel_cut_circular a
                             , syphon_ends                     b
                      WHERE
                               ST_LineLocatePoint(a.geom,b.geom)     > 0
                               AND ST_LineLocatePoint(a.geom,b.geom) < 1
    ...
    CREATE TABLE tmp.splitted_channels AS
                 (
                        SELECT
                               (ST_Linesubstrings(a.geom, b.fraction_arr)).geom as geom
                        FROM
                               deelgebied.channel_cut_circular a
                             , tmp.split_channels              b
                        WHERE
                               a.id = b.channel_cut_circular_id
                 )
    ;
    ...
    DELETE
    FROM
           deelgebied.channel_cut_circular
    WHERE
           id IN
           (
                  SELECT
                         channel_cut_circular_id
                  FROM
                         tmp.split_channels
           )
    ;
    ...
    DROP SEQUENCE
    IF EXISTS seq_channel_cut_circular_id;
        ...
        INSERT INTO deelgebied.channel_cut_circular
               (id
                    , geom
               )
        SELECT
