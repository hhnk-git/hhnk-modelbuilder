...
CREATE TABLE tmp.channel_one_profile AS
             (
                    SELECT
                           id
                         , generate_subscripts(fractions, 1) as nr
                         , array_upper(fractions,1)          as cnt
                         , unnest(fractions)                 as fraction
                         , unnest(widths)                    as width
                    FROM
                           tmp.interpolation_width
                    WHERE
                           array_upper(fractions,1) = 1
             )
;
...
CREATE TABLE tmp.fractioned AS
             (
                     ...
                     , parts AS
                       (
                              SELECT
                                   ...
                                   , b.nr       as to_nr
                                   , a.cnt      as cnt
                                   , a.fraction as from_frac
                                   , b.fraction as to_frac
                  ...
                  SELECT
                       ...
                       , from_width
                       , to_width
                       , (from_frac+to_frac)/2 as betweenfrac
                         --from frac
...
DROP SEQUENCE
IF EXISTS seq_profiel_watervlak_id;
    ...
    CREATE TABLE checks.profiel_watervlak_union AS
                 (
                      WITH geomdump AS
                           (
                                  SELECT
                                         ST_Dump(ST_Union(bufgeom)) as dumpoutput
                                  FROM
                                         checks.profiel_watervlak
                           )
                      SELECT
                             (dumpoutput).path[1] as id
                           , (dumpoutput).geom    as geom
                      FROM
                             geomdump
                 )
    ;
