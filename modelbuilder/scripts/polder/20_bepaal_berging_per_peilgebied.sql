...
CREATE TABLE tmp.channelsurface_union AS
             (
                    SELECT
                           ST_CollectionExtract(ST_Union(geom),3) as geom
                    FROM
                           deelgebied.channelsurface
             )
;
...
CREATE TABLE tmp.fdla_channel_surface AS
             (
                    SELECT
                         ...
                         , a.code
                         , a.streefpeil_bwn2
                         , ST_CollectionExtract(a.geom,3)                         as fdla_geom
                         , ST_Intersection(ST_CollectionExtract(a.geom,3),b.geom) as surface_geom
                    FROM
                           deelgebied.fixeddrainagelevelarea a
                         , tmp.channelsurface_union          b
                    WHERE
...
UPDATE
       tmp.fdla_channel_surface
SET    storage_area = ST_Area(surface_geom)
;
...
CREATE TABLE tmp.channel_streefpeil AS
             (
                    SELECT
                           a.id
                         , b.initial_waterlevel                          as start_level
                         , c.initial_waterlevel                          as end_level
                         , (b.initial_waterlevel+c.initial_waterlevel)/2 as avg_level
                         , a.the_geom                                    as geom
                    FROM
                           v2_channel          a
...
CREATE TABLE tmp.cross_sections AS
             (
                    ...
                    FROM
                         ...
                         , v2_cross_section_definition b
                         , tmp.channel_streefpeil      c
                    WHERE
...
UPDATE
       tmp.cross_sections
SET    waterdepth = GREATEST(avg_level-reference_level,0)
;
...
CREATE TABLE tmp.level_comp_unnest AS
             (
                    SELECT
                         ...
                         , arr[nr] AS height
                         , nr
                         , arr[nr]::double precision  < waterdepth as st
                         , arr[nr]::double precision >= waterdepth as gt
                    FROM
                           (
                                  SELECT *
                                       , generate_subscripts(arr, 1) AS nr
                                  FROM
                                         (
                                                SELECT
                                                       id
                                                     , waterdepth
                                                     , string_to_array(height, ' ') AS arr
                                                FROM
                                                       tmp.cross_sections
                                         )
                                         t
...
ALTER TABLE tmp.cross_sections DROP COLUMN IF EXISTS width_array
          , DROP COLUMN IF EXISTS height_array
;

ALTER TABLE tmp.cross_sections ADD COLUMN width_array double precision[]
          , ADD COLUMN height_array double precision[]
;

ALTER TABLE tmp.cross_sections DROP COLUMN IF EXISTS height_low
          , DROP COLUMN IF EXISTS height_high
          , DROP COLUMN IF EXISTS width_low
          , DROP COLUMN IF EXISTS width_high
;

ALTER TABLE tmp.cross_sections ADD COLUMN height_low double precision
          , ADD COLUMN height_high double precision
          , ADD COLUMN width_low double precision
          , ADD COLUMN width_high double precision
;

UPDATE
       tmp.cross_sections
SET    width_array  = string_to_array(width, ' ')::double precision[]
     , height_array = string_to_array(height, ' ')::double precision[]
;

UPDATE
       tmp.cross_sections a
SET    height_low  = height_array[index_low]
     , height_high = height_array[index_high]
     , width_low   = width_array[index_low]
     , width_high  = width_array[index_high]
FROM
       tmp.level_comp_indices b
WHERE
       a.id = b.id
;
...
UPDATE
       tmp.cross_sections
SET    height_low  = height_array[array_upper(height_array,1)-1]
     , height_high = height_array[array_upper(height_array,1)]
     , width_low   = width_array[array_upper(width_array,1)-1]
     , width_high  = width_array[array_upper(width_array,1)]
WHERE
       waterdepth > height_array[array_upper(height_array,1)]
;

--Indien waterdiepte = 0
UPDATE
       tmp.cross_sections
SET    height_low  = height_array[1]
     , height_high = height_array[2]
     , width_low   = width_array[1]
     , width_high  = width_array[2]
WHERE
       waterdepth = 0
;
...
SET    width_at_waterlevel =
       CASE
              WHEN (
                     ...
                     = 0
                     THEN width_low
                     ELSE width_low + LEAST(1, GREATEST(0, (waterdepth-height_low)/(height_high-height_low)))*(width_high-width_low)
       END
;

DROP SEQUENCE
IF EXISTS seq_channel_cs_join;
    ...
    CREATE TABLE tmp.channel_cs_join AS
                 (
                      WITH ordered AS
                           (
                                    SELECT
                                           ...
                                           , b.id as cs_id
                                           , a.the_geom
                                           , b.the_geom                                as cs_geom
                                           , ST_LineLocatePoint(a.the_geom,b.the_geom) as fraction
                                           , c.width_at_waterlevel
                                           , ST_Length(a.the_geom) as length
                                    FROM
                                             v2_channel                a
                                           , v2_cross_section_location b
                                           , tmp.cross_sections        c
                                    WHERE
                                             a.id     = b.channel_id
                                             AND b.id = c.id
                                    ORDER BY
                      ...
                      SELECT
                             nextval('seq_channel_cs_join') as order_id
                           , *
                      FROM
                             ordered
                 )
    ...
    CREATE TABLE tmp.interpolation AS
                 (
                          SELECT
                                 ...
                                 , array_append(array_prepend((array_agg(width_at_waterlevel))[1],array_agg(width_at_waterlevel)),(array_agg(width_at_waterlevel))[array_upper(array_agg(width_at_waterlevel),1)]) as widths
                 ...
                 (
                      WITH explode AS
                           (
                                  SELECT
                                         id
                                       , unnest(fractions) as frac
                                       , unnest(widths)    as width
                                       , length
                                       , generate_subscripts(fractions, 1) AS index
                                  FROM
                                         tmp.interpolation
                           )
                         , explode_join AS
    ...
    CREATE TABLE tmp.channel_surface_area_midpoints AS
                 (
                        SELECT
                               a.id
                             , a.surface_area
                             , ST_LineInterpolatePoint(b.the_geom, 0.5) as midpoint
                        FROM
                               tmp.channel_surface_area a
                             , v2_channel               b
                        WHERE
                               a.id = b.id
                 )
    ...
    UPDATE
           tmp.channel_surface_area_midpoints a
    SET    fdla_id = b.id
    FROM
           deelgebied.fixeddrainagelevelarea b
    WHERE
           ST_Contains(b.geom,a.midpoint)
    ;
                 ...
                 (
                      WITH modelled_surface AS
                           (
                                    SELECT
                                             fdla_id
                                           , sum(surface_area) as modelled_surface
                                    FROM
                                             tmp.channel_surface_area_midpoints
                                    GROUP BY
                                             fdla_id
                           )
                      SELECT
                           ...
                           , a.streefpeil_bwn2
                           , a.fdla_geom
                           , a.storage_area as bgt_surface
                           , b.modelled_surface
                           , (100           *(b.modelled_surface-a.storage_area))/a.storage_area as diff_percent
                           , (a.storage_area-b.modelled_surface)                                 as added_surface
                      FROM
                             tmp.fdla_channel_surface a
                           , modelled_surface         b
                      WHERE
                             a.id = b.fdla_id
                 )
    ...
    CREATE TABLE tmp.secundary_channel_connection_nodes AS
                 (
                      ...
                      FROM
                           ...
                           , primary_channels    b
                      WHERE
                             NOT ST_Intersects(a.the_geom, b.geom)
                 )
    ;
    ...
    CREATE TABLE tmp.sec_con_node_per_fdla AS
                 (
                          SELECT
                                   a.id
                                 , count(*)
                                 , array_agg(b.id)    as ids_array
                                 , min(added_surface) as added_surface
                          FROM
                                   checks.modelled_surface                a
                                 , tmp.secundary_channel_connection_nodes b
                          WHERE
                                   ST_Intersects(a.fdla_geom, b.the_geom)
                          GROUP BY
                                   a.id
                 )
    ;
    ...
    UPDATE
           tmp.sec_con_node_per_fdla
    SET    add_storage_per_node = added_surface / array_upper(ids_array,1)
    ;
    ...
    */
    ...
    SET    storage_area =
           CASE
                  WHEN storage_area           IS NULL
                         AND b.add_storage_per_node > 2
                         THEN b.add_storage_per_node
                  WHEN storage_area       IS NOT NULL
                         AND b.add_storage_per_node > 2
                         THEN storage_area+b.add_storage_per_node
                         ELSE storage_area
           END
    FROM
           tmp.sec_con_node_per_fdla b
    WHERE
           a.id = ANY(b.ids_array)
    ;
