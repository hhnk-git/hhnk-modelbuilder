...
IF EXISTS channelserial;
    ...
    IF EXISTS crsserial;
        ...
        IF EXISTS crsdefserial;
            ...
            SELECT
                     (ST_dump(ST_Linemerge(ST_UNION(geom)))).geom
                   , ST_Buffer((ST_dump(ST_Linemerge(ST_UNION(geom)))).geom , 0.5) as bufgeom
                   , channel_type_id
                   , nextval('channelserial') as channel_type_reach_id
            FROM
                     deelgebied.channel
            GROUP BY
                     channel_type_id
            ...
            CREATE INDEX deelgebied_channel_typeunion_bufgeom
            ON
                         deelgebied.channel_typeunion
            USING        gist
                         (
                                      bufgeom
                         )
            ...
            CREATE INDEX deelgebied_channel_typeunion_geom
            ON
                         deelgebied.channel_typeunion
            USING        gist
                         (
            ...
            CREATE INDEX deelgebied_tmp_sel_branches_without_structures_pointgeom
            ON
                         deelgebied.tmp_sel_branches_without_structures
            USING        gist
                         (
            ...
            CREATE INDEX deelgebied_tmp_sel_branches_without_structures_bufgeom
            ON
                         deelgebied.tmp_sel_branches_without_structures
            USING        gist
                         (
            ...
            UPDATE
                   deelgebied.tmp_sel_branches_without_structures
            SET    geom = ST_LineMerge(ST_Union(ST_MakeLine(ST_StartPoint(geom),ST_LineInterpolatePoint(geom,0.5)),ST_MakeLine(ST_LineInterpolatePoint(geom,0.5),ST_EndPoint(geom))))
            WHERE
                   ST_NPoints(geom) = 2
            ;
            ...
            UPDATE
                   deelgebied.crosssection as a
            SET    channel_type_reach_id = b.channel_type_reach_id
            FROM
                   deelgebied.channel_typeunion as b
            WHERE
            ...
            UPDATE
                   deelgebied.tmp_sel_branches_without_structures as a
            SET    channel_type_reach_id = b.channel_type_reach_id
            FROM
                   deelgebied.channel_typeunion as b
            WHERE
                   ST_Intersects(b.bufgeom,a.pointgeom)
            ;
            ...
            FROM
                   deelgebied.tmp_sel_branches_without_structures as b
            WHERE
                   ST_Intersects(b.bufgeom,a.geom)
            ;
            ...
            SELECT
                   id
                 , branch_reach_id  as channel_id
                 , cross_profile_id as definition_id
                 , bed_level        as reference_level
                 , 2                as friction_type
                 , (1.0/30)         as friction_value
                 , bank_level
                 , geom as the_geom
                 , code
                 , NULL as shape
            ...
            ON
                      ...
                      nextval('crsserial')                as id
                    , a.reach_id                          as channel_id
                    , b.cross_profile_id                  as definition_id
                    , null                                as reference_level
                    , 2                                   as friction_type
                    , (1.0/30)                            as friction_value
                    , NULL                                as bank_level
                    ...
                    , (b.code
                                || '-'
                                || a.reach_id) as code
                    , --MAKE CODE HERE IDENTIFIABLE
                      NULL::integer as shape
                    , ''            as width
                    , ''            as height
            ...
            FROM
                      ...
                      LEFT JOIN
                                ...
                                ON
                                          a.channel_type_reach_id = b.channel_type_reach_id
            WHERE
                      b.cross_profile_id IS NOT NULL
            ...
            SET    bank_level      = b.streefpeil_bwn2 +0.1
                 , reference_level = b.streefpeil_bwn2 - depth_under_fdl
            FROM
            ...
            SELECT
                 ...
                 , 2   as friction_type
                 , CASE
                          WHEN channel_type_id = 1
                                 THEN (1.0/30)
                                 ELSE (1.0/20)
                   END as friction_value
                 , --> aanname
                 ...
                 , CASE profile_type
                          WHEN 'legger'
                                 THEN a.bed_width
                                        || ' '
                                        || greatest(a.bed_width + (b.streefpeil_bwn2 - a.bed_level)*((talud_right+talud_left)),a.bed_width)
                          WHEN 'getabuleerde breedte en bodemhoogte'
                                 THEN tabulated_width
                   END as width
                 , -- <- hier wordt legger gebruikt
                   CASE profile_type
                          WHEN 'legger'
                                 THEN '0 '
                                        || greatest((b.streefpeil_bwn2 - a.bed_level),0.5)
                          WHEN 'getabuleerde breedte en bodemhoogte'
            ...
            FROM
                 ...
                 , deelgebied.fixeddrainagelevelarea as b
            WHERE
                   ST_Intersects(ST_LineInterpolatePoint(a.geom,0.1), b.geom)
            ...
            FROM
                 ...
                 , deelgebied.fixeddrainagelevelarea as b
            WHERE
                   ST_Intersects(ST_LineInterpolatePoint(a.geom,0.9), b.geom)
                   AND ST_Length(a.geom) > 40
            UNION
            SELECT
                   nextval('crsserial')    as id
            ...
            FROM
                 ...
                 , deelgebied.fixeddrainagelevelarea as b
            WHERE
                   ST_Intersects(ST_LineInterpolatePoint(a.geom,0.5), b.geom)
                   AND ST_Length(a.geom) <= 40
            ;
            
            CREATE INDEX deelgebied_tmp_channel_crs_geom
            ON
                         deelgebied.tmp_channel_crs
            USING        gist
                         (
                                      the_geom
                         )
            ...
            on
                 ...
                 , a.width
                 , a.height
                 , b.channel_type_reach_id
                 , null
            FROM
                   deelgebied.tmp_channel_crs                     as a
                 , deelgebied.tmp_sel_branches_without_structures as b
            WHERE
                   ST_Intersects(b.bufgeom,a.the_geom)
                   AND b.reach_id NOT IN
                   (
                          SELECT
                                 channel_id
                          FROM
            ...
            FROM
                      deelgebied.tmp_sel_branches_without_structures as a
                      LEFT JOIN
                                deelgebied.tmp_channel_crs as b
                                ON
                                          a.channel_type_reach_id = b.channel_type_reach_id
            WHERE
                      b.channel_type_reach_id IS NOT NULL
                      AND reach_id NOT IN
                      (
                             SELECT
                                    channel_id
            ...
            ORDER BY
                      a.reach_id ASC
                    , ST_Distance(a.pointgeom,b.the_geom) ASC
            ;
            ...
            FROM
                      ...
                      LEFT JOIN
                                deelgebied.tmp_channel_crs as b
                                ON
                                          ST_DWithin(a.geom,b.the_geom,20)
            WHERE
                      b.definition_id IS NOT NULL
                      AND reach_id NOT IN
                      (
                             SELECT
                                    channel_id
            ...
            FROM
                   deelgebied.tmp_sel_branches_without_structures as b
            WHERE
                   a.channel_id = b.reach_id
                   AND ST_Linelocatepoint(b.geom,a.the_geom) NOT BETWEEN 0.01 AND 0.99
            ;
            ...
            AS
                ...
                BEGIN
                    SELECT
                           COUNT(*)::integer
                    FROM
                           deelgebied.tmp_sel_branches_without_structures AS c
                    WHERE
                           (
                                  $2 = c.reach_id
                           )
                    ...
                    IF result > 0 THEN
                        UPDATE
                               deelgebied.tmp_sel_branches_without_structures
                        SET    geom = ST_LineMerge( ST_SnapToGrid( ST_Union( ST_LineSubstring(tmp_sel_branches_without_structures.geom, 0, ST_LineLocatePoint(tmp_sel_branches_without_structures.geom, $1)), ST_LineSubstring(tmp_sel_branches_without_structures.geom, ST_LineLocatePoint(tmp_sel_branches_without_structures.geom, $1), 1) ),0,0,0.05,0.05))
                        WHERE
                               (
                                      $2 = tmp_sel_branches_without_structures.reach_id
                               )
                        ;
            ...
            FUNCTION intersects_channel
                ...
                SELECT
                       intersects_channel(the_geom,channel_id::integer)
                FROM
                       deelgebied.tmp_v2_cross_section_location
                ; --Duurt 8 minuten voor heel hhnk
