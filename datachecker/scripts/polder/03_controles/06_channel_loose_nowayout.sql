...
SELECT nextval('serial') as id, (ST_dump(ST_Union(bufgeom))).geom, b.polder_id::numeric
FROM checks.channel_linemerge as a, checks.polder as b
WHERE ST_Intersects(a.bufgeom,b.geom)
GROUP BY b.polder_id;
CREATE INDEX checks_channel_loose_geom ON checks.channel_loose USING gist(geom); --160sec
...
WITH join_linemerge_loose AS(
	SELECT a.id, a.polder_id, b.geom as linegeom
	FROM checks.channel_loose a
	LEFT JOIN checks.channel_linemerge b
	ON ST_Contains(a.geom,b.geom)
)
SELECT id, SUM(ST_Length(linegeom)) as lengte,  ST_Union(linegeom)
FROM join_linemerge_loose 
GROUP BY id 
);
...
UPDATE checks.channel_loose a SET lengte = ST_Area(geom)/0.2;-- b.lengte FROM (SELECT * FROM tmp.channel_loose_length) b WHERE a.id = b.id;
UPDATE checks.channel_loose SET pointgeom = ST_PointOnSurface(geom);
CREATE INDEX checks_channel_loose_pointgeom ON checks.channel_loose USING gist(pointgeom);
...
DELETE FROM checks.channel_loose as a
USING checks.afvoerkunstwerken as b
WHERE ST_Intersects(a.geom,b.geom); --1sec
...
SELECT a.* 
FROM checks.channel as a, checks.channel_loose b
...
CREATE TABLE tmp.wrong_channels AS 
    SELECT a.geom, a.bufgeom, a.id, a.type as channel_type_id, 'kruising_zonder_kunstwerk'::varchar(250) as opmerking
    FROM checks.channel as a, checks.kruising_zonder_kunstwerk as b
	WHERE  ST_Intersects(b.pointgeom,a.bufgeom)
...
SELECT DISTINCT ON (a.id) a.geom,a.bufgeom, a.id, a.channel_type_id, concat('duiker/sifon op watergang wordt weggegooid, reden: ', opmerking) as opmerking
FROM checks.channel a, tmp.unusable_culvert_endpoints b
WHERE ST_Intersects(a.bufgeom,b.geom);

--Voeg kanalen toe aan tmp.wrong_channels die intersecten met vispassages (weir type = 5)
INSERT INTO tmp.wrong_channels
SELECT DISTINCT ON (a.id) a.geom,a.bufgeom, a.id, a.channel_type_id, 'vispassage' as opmerking
FROM checks.channel a, checks.weirs b
WHERE ST_Intersects(a.bufgeom,b.geom)
AND b.type LIKE '5';

-- voeg kanalen toe in foute peilafwijkingen
INSERT INTO tmp.wrong_channels
SELECT DISTINCT ON (a.id) a.geom,a.bufgeom, a.id, a.channel_type_id, 'onderbemaling zonder gemaal' as opmerking
FROM checks.channel a, checks.fdla_sp_nowayout as b
WHERE b.opmerking_peil LIKE '%onderbemaling zonder gemaal%' AND ST_Intersects(a.geom,b.geom)
;
INSERT INTO tmp.wrong_channels
SELECT DISTINCT ON (a.id) a.geom,a.bufgeom, a.id, a.channel_type_id, 'opmaling zonder stuw of duiker' as opmerking
FROM checks.channel a, checks.fdla_sp_nowayout as b
WHERE b.opmerking_peil LIKE '%opmaling zonder stuw of duiker%' AND ST_Intersects(a.geom,b.geom)
;

-- voeg kanalen toe in foute peilgebieden
INSERT INTO tmp.wrong_channels
SELECT DISTINCT ON (a.id) a.geom,a.bufgeom, a.id, a.channel_type_id, 'laagste peilgebied zonder gemaal' as opmerking
FROM checks.channel a, checks.fdla_sp_nowayout as b
WHERE b.opmerking_peil LIKE '%laagste peilgebied zonder gemaal%' AND ST_Intersects(a.geom,b.geom)
;
INSERT INTO tmp.wrong_channels
SELECT DISTINCT ON (a.id) a.geom,a.bufgeom, a.id, a.channel_type_id, 'hoogste peilgebied zonder stuw of duiker' as opmerking
FROM checks.channel a, checks.fdla_sp_nowayout as b
WHERE b.opmerking_peil LIKE '%hoogste peilgebied zonder stuw of duiker%' AND ST_Intersects(a.geom,b.geom)
;
...
CREATE TABLE tmp.channel_nowayout AS
    SELECT * FROM checks.channel 
    WHERE id NOT IN (
    SELECT id FROM tmp.wrong_channels
    ); --119sec
CREATE INDEX tmpchannel_nowayout_bufgeom ON tmp.channel_nowayout USING gist(bufgeom);
...
SELECT nextval('serial') as id, (ST_dump(ST_Union(bufgeom))).geom, b.polder_id::numeric
FROM tmp.channel_nowayout as a, checks.polder as b
WHERE ST_Intersects(a.bufgeom,b.geom)
GROUP BY b.polder_id
;  --119sec
CREATE INDEX checks_channel_nowayout_geom ON checks.channel_nowayout USING gist(geom);

-- 2.4) Verwijder delen die wel verbonden zijn met afvoerkunstwerken
DELETE FROM checks.channel_nowayout as a
USING checks.afvoerkunstwerken as b
WHERE ST_Intersects(a.geom,b.geom);

-- 2.5) Voeg de foutieve segmenten toe aan channel_nowayout;
INSERT INTO checks.channel_nowayout(geom)
SELECT bufgeom from tmp.wrong_channels;
...
WITH join_linemerge_nowayout AS(
	SELECT a.id, a.polder_id, b.geom as linegeom
	FROM checks.channel_nowayout a
	LEFT JOIN checks.channel_linemerge b
	ON ST_Contains(a.geom,b.geom)
)
SELECT id, SUM(ST_Length(linegeom)) as lengte,  ST_Union(linegeom)
FROM join_linemerge_nowayout 
GROUP BY id 
);
...
UPDATE checks.channel_nowayout a SET lengte = ST_Area(geom)/0.2; --b.lengte FROM (SELECT * FROM tmp.channel_nowayout_length) b WHERE a.id = b.id;
UPDATE checks.channel_nowayout SET pointgeom = ST_PointOnSurface(geom);
CREATE INDEX checks_channel_nowayout_pointgeom ON checks.channel_nowayout USING gist(pointgeom);
...
SELECT a.*
FROM checks.channel as a, checks.channel_nowayout b
WHERE ST_contains(b.geom,a.geom)
;
...
CREATE TABLE tmp.checks_channel AS SELECT 
	id,
	geom as geom,
...
CREATE INDEX checks_channel_geom_index ON tmp.checks_channel USING gist(geom);
DROP INDEX IF EXISTS tmp.checks_channel_end_index;
CREATE INDEX checks_channel_end_index ON tmp.checks_channel USING gist(startpunt);
DROP INDEX IF EXISTS tmp.checks_channel_start_index;
CREATE INDEX checks_channel_start_index ON tmp.checks_channel USING gist(eindpunt);

--Vul tabel met start- en eindpunten
DROP TABLE IF EXISTS tmp.channel_endpoints;
CREATE TABLE tmp.channel_endpoints AS(
	SELECT id as channel_id, startpunt as geom
	FROM tmp.checks_channel
	
	UNION ALL
	
	SELECT id as channel_id, eindpunt as geom
	FROM tmp.checks_channel
	);
...
CREATE TABLE tmp.vertex_midpoints AS(
	WITH vertex_dump AS(
		SELECT id as channel_id, ST_NPoints(geom) as numpoints, ST_DumpPoints(geom) as vertex
		FROM tmp.checks_channel
	)
	SELECT channel_id, (vertex).geom as geom
	FROM vertex_dump
	WHERE (vertex).path[1] > 1
	AND (vertex).path[1] < numpoints
);
...
CREATE TABLE tmp.dead_ends AS(
	SELECT nextval('seq_dead_end') as id,
	NULL::integer as channel_id,
	geom
	FROM tmp.channel_endpoints
	GROUP BY geom
	HAVING COUNT(*)=1
);
...
CREATE INDEX tmp_dead_ends_geom ON tmp.dead_ends USING gist(geom);

--Verwijder doodlopende punten die wel op een midpoint (vertex) liggen
DELETE FROM tmp.dead_ends a USING tmp.vertex_midpoints b WHERE ST_Intersects(a.geom,b.geom);

-- dead_end koppelen aan een checks.channel
UPDATE tmp.dead_ends a SET channel_id = b.id
FROM tmp.checks_channel b
WHERE ST_Intersects(a.geom,b.geom);
...
CREATE TABLE tmp.wrong_dead_ends AS(
	SELECT a.id, a.geom, a.channel_id
	FROM tmp.dead_ends a, tmp.channel_endpoints b
...
DELETE FROM tmp.wrong_dead_ends a USING tmp.checks_channel b WHERE a.channel_id = b.id AND ST_Length(b.geom) < 1;
...
WHERE id IN (SELECT channel_id FROM tmp.wrong_dead_ends)
;
