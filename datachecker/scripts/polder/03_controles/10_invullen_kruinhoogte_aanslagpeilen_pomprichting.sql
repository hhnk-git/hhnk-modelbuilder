...
;
CREATE INDEX tmp_peilgebiedgrenzen_geom ON tmp.peilgebiedgrenzen USING gist(geom);
...
SELECT DISTINCT ON (a.id) a.*, ST_Startpoint(a.geom) as startpunt, ST_Endpoint(a.geom) as eindpunt
FROM checks.channel_linemerge as a 
WHERE a.id IN (
	SELECT channel_code::integer
	FROM checks.pumpstation
	)
; 
CREATE INDEX tmp_channel_kunstwerk_geom ON tmp.channel_kunstwerk USING gist(geom);
CREATE INDEX tmp_channel_kunstwerk_start ON tmp.channel_kunstwerk USING gist(startpunt);
CREATE INDEX tmp_channel_kunstwerk_eind ON tmp.channel_kunstwerk USING gist(eindpunt);
...
SELECT DISTINCT ON (a.id) a.*, 
	b.streefpeil_bwn2 as startpeil, b.id as startid,
	c.streefpeil_bwn2 as eindpeil, c.id as eindid,
	CASE WHEN b.streefpeil_bwn2 = c.streefpeil_bwn2 THEN 'crest/startlevel onbetrouwbaar'
		WHEN b.streefpeil_bwn2 IS NULL OR c.streefpeil_bwn2 IS NULL THEN 'crest/startlevel onbetrouwbaar'
	END as opmerking,
	CASE WHEN b.streefpeil_bwn2 < c.streefpeil_bwn2 THEN 'start'
		WHEN b.streefpeil_bwn2 > c.streefpeil_bwn2 THEN 'eind'
	END as lage_peil
FROM tmp.channel_kunstwerk as a
...
SET start_level = 
	CASE WHEN b.lage_peil = 'start' THEN b.startpeil+0.02
		WHEN b.lage_peil = 'eind' THEN b.eindpeil+0.02
		ELSE least(b.startpeil,b.eindpeil)+0.02
	END, 
	start_point_id = b.startid, end_point_id = b.eindid,
	opmerking = concat_ws(',',a.opmerking,b.opmerking)
FROM tmp.channel_kunstwerk_peil as b
WHERE a.channel_code::integer = b.id
;
...
UPDATE checks.pumpstation as a
SET start_level = b.streefpeil_bwn2+0.02, stop_level = b.streefpeil_bwn2  - 0.03, 
...
WHERE a.start_level IS NULL AND ST_DWithin(a.geom,b.geom,2);
...
FROM hdb.stuwen_op_peilgrens b
...
FROM tmp.weir_level_radius AS b
WHERE a.code LIKE b.weir_code
AND (crest_level IS NULL 
	OR
	b.max_streefpeil > crest_level)
AND type_function != 1;
...
UPDATE checks.weirs AS a
SET 
    crest_level = b.max_streefpeil + 0.5,
