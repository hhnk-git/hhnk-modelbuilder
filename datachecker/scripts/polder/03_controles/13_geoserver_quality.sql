...
UPDATE checks.crossprofile
SET isusable = 0
WHERE opmerking LIKE ANY(ARRAY['%profielhoogte<0.1m%','%profielbreedte<2m%']);
...
SET isusable = 0
WHERE opmerking LIKE ANY(ARRAY['%gw_pro dubbel ingetekend%','%koppel niet mogelijk obv geom of code%','%gw_pro zigzag ingetekend%']);
...
;
UPDATE checks.polder SET prct_pump_error_primair = 0
WHERE prct_pump_error_primair IS NULL;
...
WHERE a.polder_id = b.polder_id
;
UPDATE checks.polder SET prct_pump_error_non_primair = 0
WHERE prct_pump_error_non_primair IS NULL;
...
WHERE a.polder_id = b.polder_id
;
UPDATE checks.polder SET prct_culvert_error_primair = 0
WHERE prct_culvert_error_primair IS NULL;
...
WHERE a.polder_id = b.polder_id
;
UPDATE checks.polder SET prct_culvert_error_non_primair = 0
WHERE prct_culvert_error_non_primair IS NULL;
...
WHERE a.polder_id = b.polder_id
;
UPDATE checks.polder SET prct_weirs_error_primair = 0
WHERE prct_weirs_error_primair IS NULL;
...
WHERE a.polder_id = b.polder_id
;
UPDATE checks.polder SET prct_weirs_error_non_primair = 0
WHERE prct_weirs_error_non_primair IS NULL;
...
CREATE TABLE checks.kruising AS(
	SELECT 
		a.*, 
		b.polder_id,
		CASE WHEN a.channel_type_id = 1 THEN 1 ELSE 0 END AS primair
	FROM(
		SELECT
			DISTINCT ON(ST_INTERSECTION(x.geom,y.geom))
			x.id as channel_id,
			x.channel_type_id,
	...
	LEFT JOIN
		checks.polder AS b
		ON ST_INTERSECTS(a.geom_kruising,b.geom)
);
...
FROM (
	select a.polder_id, count(b.*) 
	from checks.polder a, checks.kruising b 
	where channel_type_id = 1 and 
		ST_Intersects(a.geom,b.geom_kruising)
	GROUP BY a.polder_id
	) as b
WHERE a.polder_id = b.polder_id
;
...
UPDATE checks.control_table SET structure_isusable = structure_code IN (
SELECT code FROM
checks.weirs WHERE isusable=1
UNION
SELECT code FROM
checks.pumpstation WHERE isusable=1
UNION
SELECT code FROM
checks.culvert WHERE isusable=1);


UPDATE checks.control_table SET is_usable = (unique_code AND structure_exists AND correct_measurement_series AND structure_isusable);

UPDATE checks.control_table
SET opmerking = concat_ws(',',opmerking,'geen unieke kunstwerk code')
WHERE NOT unique_code;

UPDATE checks.control_table
SET opmerking = concat_ws(',',opmerking,'kunstwerk niet gevonden')
WHERE NOT structure_exists;

UPDATE checks.control_table
SET opmerking = concat_ws(',',opmerking,'meetwaardes niet volledig op/aflopend')
WHERE NOT correct_measurement_series;

UPDATE checks.control_table
SET opmerking = concat_ws(',',opmerking,'kunstwerk is niet bruikbaar')
WHERE NOT structure_isusable;
