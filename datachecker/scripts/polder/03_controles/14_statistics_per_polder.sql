...
FROM
    (
        ...
        FROM
            checks.polder as a
            LEFT JOIN
                checks.afvoerkunstwerken as b
                ON
                    ST_Intersects(a.geom,b.linegeom)
        GROUP BY
            a.polder_id::numeric
    )
    as b
...
update
    checks.polder a
set cnt_pump_error_primair = count
FROM
    (
        select
            a.polder_id
        ...
        where
            ...
            (
                NOT b.on_channel
                OR b.opmerking LIKE '%meerdere%'
                OR b.opmerking LIKE '%niet op peilgrens%'
            )
...
update
    checks.polder a
set cnt_pump_error_non_primair = count
FROM
    (
        select
            a.polder_id
...
update
    checks.polder a
set cnt_culvert_error_primair = count
FROM
    (
        select
            a.polder_id
        ...
        where
            ...
            (
                ...
                OR b.opmerking LIKE '%meerdere%'
                OR b.opmerking LIKE '%niet op peilgrens%'
                OR b.opmerking LIKE '%sifon in brondata niet exact op watergang%'
            )
            and ST_Intersects(a.geom,b.geom)
        GROUP BY
            a.polder_id
...
update
    checks.polder a
set cnt_weirs_error_primair = count
FROM
    (
        select
            a.polder_id
...
update
    checks.polder a
set cnt_weirs_error_non_primair = count
FROM
    (
        select
            a.polder_id
...
update
    checks.polder a
set cnt_kruising_error_primair = count
FROM
    (
        select
            a.polder_id
        ...
        from
            checks.polder                    a
          , checks.kruising_zonder_kunstwerk b
        where
            channel_type_id = 1
            and ST_Intersects(a.geom,b.pointgeom)
        GROUP BY
            a.polder_id
    )
    as b
...
update
    checks.polder a
set cnt_kruising_error_non_primair = count
FROM
    (
        select
            a.polder_id
...
WITH alle AS
    (
        SELECT
            b.polder_id
          , sum(ST_Length(a.geom)) as alle_lengte
        FROM
            checks.channel as a
          , checks.polder  as b
        WHERE
            ST_Intersects(a.pointgeom,b.geom)
        GROUP BY
            b.polder_id
    )
  , loose AS
  ...
  , prcnt AS
    (
        SELECT
            a.polder_id
          , CASE
                WHEN b.loose_lengte IS NULL
                    THEN 0
                    ELSE least(round(((b.loose_lengte/a.alle_lengte)*100)::numeric,2),100)
            END as looseprcnt
          , CASE
                WHEN c.nowayout_lengte IS NULL
                    THEN 0
                    ELSE least(round(((c.nowayout_lengte/a.alle_lengte)*100)::numeric,2),100)
            END as nowayoutprcnt
        FROM
            alle as a
            LEFT JOIN
                loose as b
