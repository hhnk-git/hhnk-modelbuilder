    ...
    (
        WITH kunstwerken_puntgeometrie AS
            (
                ...
                UNION ALL
                SELECT
                    'damo_ruw.vispassage' as tabel
                  , f.code
                  , f.wkb_geometry
                FROM
                    damo_ruw.vispassage f
            )
        ...
        SELECT
            a.*
        FROM
            kunstwerken_puntgeometrie     a
          , dubbele_kunstwerken_geometrie b
        WHERE
...
UNION
SELECT
    'damo_ruw.duikersifonhevel' as tabel
  , l.code
  , l.wkb_geometry
FROM
...
WHERE
    ...
    (
        SELECT
            a.code
        FROM
            tmp.dubbele_kunstwerken a
            LEFT JOIN
                tmp.dubbele_kunstwerken b
                ON
                    a.wkb_geometry    =b.wkb_geometry
                    AND a.code NOT LIKE b.code
        WHERE
            a.tabel     LIKE 'damo_ruw.brug'
            AND b.tabel LIKE 'damo_ruw.stuw'
    )
;
        ...
        FROM
            ...
            LEFT JOIN
                ...
                ON
                    ...
                    AND a.code NOT LIKE b.code
            LEFT JOIN
                checks.pumpstation c
                ON
                    b.code = c.code
        WHERE
            a.tabel     LIKE 'damo_ruw.stuw'
            AND b.tabel LIKE 'damo_ruw.gemaal'
            AND c.type  LIKE '2'
    )
;
...
WHERE
    ...
    (
        ...
        WHERE
            a.tabel     LIKE 'damo_ruw.stuw'
            AND b.tabel LIKE 'damo_ruw.vastedam'
    )
;
...
CREATE TABLE checks.dubbele_kunstwerken AS
SELECT
    tabel
  , code
...
FROM
    tmp.dubbele_kunstwerken
;

INSERT INTO checks.dubbele_kunstwerken
    (tabel
      , code
      , opmerking
      , geom_point
