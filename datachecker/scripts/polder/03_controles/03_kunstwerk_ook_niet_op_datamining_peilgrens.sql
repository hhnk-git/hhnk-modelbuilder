...
SELECT ST_Buffer(ST_ExteriorRing((ST_Dump(wkb_geometry)).geom),0.1) as geom
FROM fixed_data.datamining_ahn3;
DROP INDEX IF EXISTS tmp.tmp_peilgrenzen_geom;
CREATE INDEX tmp_peilgrenzen_geom ON tmp.peilgrenzen_datamining USING gist(geom);
...
CREATE INDEX checks_pumpstation_idx ON checks.pumpstation USING gist(geom);
DROP INDEX IF EXISTS checks.checks_weirs_idx;
CREATE INDEX checks_weirs_idx ON checks.weirs USING gist(geom);
DROP INDEX IF EXISTS checks.checks_culvert_idx;
CREATE INDEX checks_culvert_idx ON checks.culvert USING gist(geom);
...
WHERE opmerking like '%niet op peilgrens%'
and code IN (
SELECT * FROM tmp.shortlist
	)
; -- 669/2090
...
CREATE INDEX checks_culvert_id ON checks.culvert USING btree(id);
