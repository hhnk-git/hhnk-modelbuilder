...
UPDATE
    checks.control_table
SET unique_code = structure_code NOT IN
    (
        SELECT
            structure_code
        FROM
...
UPDATE
    checks.control_table
SET structure_exists = structure_code IN
    (
        SELECT
            code
        FROM
    ...
    (
        WITH unnested AS
            (
                SELECT
                    structure_code
                  , measure_values[elem_nr] as m_value
                  , elem_nr
                FROM
                    (
                        SELECT *
                          , generate_subscripts(measure_values,1) AS elem_nr
                        FROM
                            checks.control_table
                    )
                    t
          ...
          , ordered_increment AS
            (
                SELECT
                  ...
                  , elem_nr
                  , m_value
                  , lag(m_value) OVER (PARTITION BY structure_code ORDER BY
                                       elem_nr)
                FROM
                    unnested
            )
...
UPDATE
    checks.control_table
SET correct_measurement_series = structure_code IN
    (
        SELECT
            structure_code
        FROM
            tmp.control_measurement_increment
        WHERE
            correct_measurement_series
    )
;
...
CREATE TABLE tmp.table_control AS
    (
          ...
          , unnested_concat AS
            (
                SELECT
                    structure_code
                  , CASE action_type
                        WHEN 'set_crest_level'
                            THEN concat(measurement_value::text,';', control_value)
                        WHEN 'set_discharge_coefficients'
                            THEN concat(measurement_value::text,';', control_value, ' ' ,control_value)
                            ELSE NULL
                    END as step_control
                FROM
                    unnested
        ...
        SELECT
            structure_code
          , string_agg(step_control,'#') as table_control
        FROM
            unnested_concat
        GROUP BY
            structure_code
...
SET table_control_string = b.table_control
FROM
    tmp.table_control b
WHERE
    a.structure_code = b.structure_code
;
