...
CREATE TABLE tmp.culvert_start_fdla_id AS
    (
        SELECT
            a.code
          , array_agg(b.id) as fdla_array
        FROM
            checks.culvert                a
          , checks.fixeddrainagelevelarea b
            --WHERE ST_intersects(st_startpoint(a.geom),ST_Buffer(b.geom,0.01))
        WHERE
            ST_dwithin(st_startpoint(a.geom),b.geom,0.01)
        GROUP BY
            a.code
    )
;
...
CREATE TABLE tmp.culvert_end_fdla_id AS
    (
        ...
        FROM
          ...
          , checks.fixeddrainagelevelarea b
        WHERE
            ST_dwithin(st_endpoint(a.geom),b.geom,0.01)
        GROUP BY
            a.code
    )
;
...
UPDATE
    checks.culvert a
SET fixeddrainagelevelarea_array_1 = fdla_array
FROM
    tmp.culvert_start_fdla_id b
WHERE
    a.code = b.code
;
...
UPDATE
    checks.culvert a
SET fixeddrainagelevelarea_array_2 = fdla_array
FROM
    tmp.culvert_end_fdla_id b
WHERE
    a.code = b.code
;
...
UPDATE
    checks.culvert a
SET fixeddrainagelevelarea_id_1 =
    CASE
        WHEN array_length(fixeddrainagelevelarea_array_1,1) = 1
            THEN fixeddrainagelevelarea_array_1[1]
        WHEN array_length(fixeddrainagelevelarea_array_1,1) = 2
            THEN (array_remove(fixeddrainagelevelarea_array_1,fixeddrainagelevelarea_array_2[1]))[1]
        WHEN array_length(fixeddrainagelevelarea_array_1,1) > 2
            THEN (array_remove(fixeddrainagelevelarea_array_1,fixeddrainagelevelarea_array_2[1]))[1]
    END
  , fixeddrainagelevelarea_id_2 =
    CASE
        WHEN array_length(fixeddrainagelevelarea_array_2,1) = 1
            THEN fixeddrainagelevelarea_array_2[1]
        WHEN array_length(fixeddrainagelevelarea_array_2,1) = 2
            THEN (array_remove(fixeddrainagelevelarea_array_2,fixeddrainagelevelarea_array_1[1]))[1]
        WHEN array_length(fixeddrainagelevelarea_array_2,1) > 2
            THEN (array_remove(fixeddrainagelevelarea_array_2,fixeddrainagelevelarea_array_1[1]))[1]
    END
;
...
ALTER TABLE checks.culvert DROP COLUMN IF EXISTS on_fdla_border
;

ALTER TABLE checks.culvert ADD COLUMN on_fdla_border BOOLEAN
;
...
WHERE
    a.fixeddrainagelevelarea_array_1 <> a.fixeddrainagelevelarea_array_2
    AND ST_Intersects(a.geom,b.geom)
;

UPDATE
    checks.culvert a
SET on_fdla_border = FALSE
FROM
    checks.weirs as b
WHERE
    a.on_fdla_border is NULL
    or ST_DWithin(a.geom,b.geom,0.1)
;
...
UPDATE
    checks.culvert a
SET on_fdla_border = NULL
FROM
    checks.weirs as b
WHERE
    a.on_fdla_border
    AND b.opmerking IS NULL
    AND ST_DWithin(a.geom,b.geom,0.1)
;
...
ALTER TABLE checks.culvert DROP COLUMN IF EXISTS on_polder_border
;

ALTER TABLE checks.culvert ADD COLUMN on_polder_border boolean
;

UPDATE
    checks.culvert a
SET on_polder_border = True
FROM
...
WHERE
    ...
    AND
    (
        NOT level_seperator_indicator
        OR level_seperator_indicator IS NULL
    )
    AND NOT attached
    AND NOT closeable
    AND NOT hdb_open
...
FROM
    (
        SELECT
            b.id
          , GREATEST(c.streefpeil_bwn2,d.streefpeil_bwn2) as maxpeil
        FROM
            checks.culvert b
            LEFT JOIN
                checks.fixeddrainagelevelarea c
                ON
                    b.fixeddrainagelevelarea_id_1 = c.id
            LEFT JOIN
                checks.fixeddrainagelevelarea d
                ON
                    b.fixeddrainagelevelarea_id_2 = d.id
    )
    e
WHERE
    a.id = e.id
...
UPDATE
    checks.culvert a
SET bed_level_upstream = greatest(bed_level_upstream, bed_level_downstream, maxpeil)
FROM
    (
        SELECT
            b.id
...
UPDATE
    checks.culvert
SET opmerking                      = concat_ws(',',opmerking,'afsluitbare inlaat op peilgrens')
  , discharge_coefficient_positive = 0.0
  , discharge_coefficient_negative = 0.0
WHERE
    on_fdla_border
