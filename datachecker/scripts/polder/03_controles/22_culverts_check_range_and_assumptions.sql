...
WHERE
    ...
    AND
    (
        a.opmerking   NOT LIKE ALL (ARRAY['%niet afsluitbaar op peilgrens%','%afsluitbare inlaat op peilgrens%','%afsluitbare afvoer op peilgrens%'])
...
WHERE
    ...
    AND
    (
        a.bed_level_upstream > b.streefpeil_bwn2
        OR a.bed_level_upstream NOT BETWEEN -20 AND 20
    )
    AND
    (
        a.opmerking   NOT LIKE ALL (ARRAY['%niet afsluitbaar op peilgrens%','%afsluitbare inlaat op peilgrens%','%afsluitbare afvoer op peilgrens%','%(hdb:open)%'])
        OR a.opmerking IS NULL
    )
    AND
    (
...
WHERE
    ...
    AND
    (
        a.bed_level_downstream > b.streefpeil_bwn2
        OR a.bed_level_downstream NOT BETWEEN -20 AND 20
    )
    AND
    (
        NOT level_seperator_indicator
