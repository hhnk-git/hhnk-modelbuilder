-- de terror............... sommige profielen zijn 5x overelkaar getekend. Bijvoorbeeld pro_id 1340 (ten westen van Volemdam, tussen volemdam en de golfbaan(?)) 
-- zoom maar eens helemaal in op dat profiel.... in dit script wordt hiermee rekening gehouden. Koste wel ontzettend veel tijd voordat ik doorhad dat dat de fout was... 
-- dit komt op 21 plekken voor in heel hhnk. --> select * from tmp.tabulated_table where numgeometries >1 order by numGeometries desc;

-- copy crossprofile to checks schema
DROP TABLE IF EXISTS checks.crossprofile;
CREATE TABLE checks.crossprofile AS
SELECT * FROM nxt.crossprofile;
...
UPDATE checks.crossprofile a 
        SET max_height = b.max_height,
        max_width = b.max_width,
        min = b.min,
        max = b.max,
        nr_input_points = b.nr_input_points
    FROM tmp.tabulated_table b 
    WHERE a.id = b.pro_id;

-- add opmerking
UPDATE checks.crossprofile SET opmerking = 'profielhoogte<0.3m' WHERE max_height < 0.3;
UPDATE checks.crossprofile SET opmerking = concat_ws(',',opmerking,'profielbreedte<2m') WHERE max_width <2;
UPDATE checks.crossprofile SET opmerking = concat_ws(',',opmerking,'lage bodem?[mnap]?') WHERE min <-9;
UPDATE checks.crossprofile SET opmerking = concat_ws(',',opmerking,'hoge bodem[mnap]?') WHERE min >3;
UPDATE checks.crossprofile SET opmerking = concat_ws(',',opmerking,'lage bovenkant[mnap]?') WHERE max <-6;
UPDATE checks.crossprofile SET opmerking = concat_ws(',',opmerking,'hoge bovenkant[mnap]?') WHERE max >6;
UPDATE checks.crossprofile SET opmerking = concat_ws(',',opmerking,'aantal profielpunten<5') WHERE nr_input_points <5;

-- copy crosssection to checks schema
DROP TABLE IF EXISTS checks.crosssection;
CREATE TABLE checks.crosssection AS
SELECT id, cross_profile_id, cross_profile_id::varchar as code, channel_id, prw_id, friction_type, friction_value, 
       distance_on_channel, bank_level, bed_level, bed_width, width, slope_left, 
...
UPDATE checks.crosssection SET isusable = 
        CASE 
            WHEN opmerking LIKE '%koppel niet mogelijk obv geom of code%' OR geom IS NULL THEN 0
            ELSE 1
        END;
        
-- 5.2) Geometrie is niet bruikbaar als op die plek een invalid crossprofile is (profile met opmerking)
UPDATE checks.crosssection SET isusable = 0
        WHERE cross_profile_id IN (
            SELECT id 
            FROM checks.crossprofile 
            WHERE opmerking LIKE 'profielbreedte<2m'
            OR opmerking LIKE 'profielhoogte<0.3m'
            OR opmerking LIKE 'aantal profielpunten<5'
            );

-- 5.3 isusable moet 0 of 1 zijn (en niet NULL)            
UPDATE checks.crosssection SET isusable = 0 WHERE isusable IS NULL;
...
UPDATE checks.crossprofile SET isusable = 1 WHERE id NOT IN (
    SELECT id 
    FROM checks.crossprofile 
    WHERE opmerking LIKE 'profielbreedte<2m'
    OR opmerking LIKE 'profielhoogte<0.3m'
...
UPDATE checks.crossprofile SET isusable = 0
    WHERE id IN (
            SELECT cross_profile_id 
            FROM checks.crosssection
            WHERE isusable = 0)
            ;
-- 5.6) isusable moet 0 of 1 zijn (en niet NULL)
UPDATE checks.crossprofile SET isusable = 0 WHERE isusable IS NULL;




            
SELECT COUNT(*) FROM checks.crosssection;           
SELECT COUNT(*) FROM checks.crosssection WHERE isusable = 1;
-- 10527 vd 19295 zijn bruikbaar 

SELECT COUNT(*) FROM checks.crossprofile;
SELECT COUNT(*) FROM checks.crossprofile WHERE isusable = 1;
-- 10527 vd 19295 zijn bruikbaar

-- zijn crossprofiles en crosssection wel consistent met elkaar (mbt isusable)?
SELECT * FROM checks.crossprofile 
	WHERE isusable <> 1 
	AND id NOT IN (
		SELECT cross_profile_id FROM checks.crosssection WHERE isusable <> 1
		);

-- Andersom: zijn crosssection en crossprofiles wel consistent met elkaar (mbt isusable)?
SELECT * FROM checks.crosssection 
	WHERE isusable <> 1 
	AND cross_profile_id NOT IN (
		SELECT id FROM checks.crossprofile WHERE isusable <> 1
		);	
...
FROM checks.channel WHERE tabulated_height IS NOT NULL
)
SELECT a.id, a.idx as aidx, b.idx as bidx, a.height as aheight, b.height as bheight FROM unnested a, unnested b
WHERE a.id = b.id
AND a.idx+1 = b.idx
AND a.height ~ '\d+(\.\d+)?'
AND b.height ~ '\d+(\.\d+)?'
...
UPDATE checks.channel SET profile_valid =
(
tabulated_width ~ '^\d+(\.\d+)?(\s\d+(\.\d+)?)*\s\d+(\.\d+)?$'
...
AND (LENGTH(tabulated_width)-LENGTH(REPLACE(tabulated_width,' ', '')))=(LENGTH(tabulated_height)-LENGTH(REPLACE(tabulated_height,' ', '')))
AND id NOT IN (SELECT id FROM tmp.badly_ordered_tabulated_profiles)
);
...
UPDATE checks.channel SET profile_type =
CASE WHEN profile_valid
