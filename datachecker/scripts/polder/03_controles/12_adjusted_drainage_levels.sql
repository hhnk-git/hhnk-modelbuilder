...
CREATE INDEX fdla_EXPLODE_geom
on
    tmp.fdla_EXPLODE
using gist
    (
...
SELECT
    a.*
  , -- hierin zit dus ook type (1=peilgebied, 2=peilafwijking), en name in (waardoor we kunnen filteren op %boezem%)
...
CREATE TABLE tmp.peilgebieden_neighbours_1m AS
SELECT DISTINCT
ON
    (
        id
    ...
    *
FROM
    tmp.tmp_peilgebieden_neighbours
;

DROP INDEX IF EXISTS tmp.peilgebieden_neighbours_1m_geom;
CREATE INDEX peilgebieden_neighbours_1m_geom
on
    tmp.peilgebieden_neighbours_1m
using gist
    (
        geom
    )
...
WHERE
    (
        opmaling
        OR onderbemaling
        OR peilgebied_hoogst
        OR peilgebied_laagst
    )
;

-- 4 sec
DROP INDEX IF EXISTS tmp.peilgebieden_neighbours_1m_bufgeom;
CREATE INDEX peilgebieden_neighbours_1m_bufgeom
on
    tmp.peilgebieden_neighbours_1m
using gist
    (
...
UPDATE
    tmp.peilgebieden_neighbours_1m a
...
UPDATE
    tmp.peilgebieden_neighbours_1m
SET opmerking_peil = 'onderbemaling zonder gemaal'
  , isusable       = FALSE
WHERE
    type = 2
    AND onderbemaling
...
UPDATE
    tmp.peilgebieden_neighbours_1m
SET opmerking_peil = 'opmaling zonder stuw of duiker'
  , isusable       = FALSE
WHERE
    type = 2
    AND opmaling
...
UPDATE
    tmp.peilgebieden_neighbours_1m
SET opmerking_peil = 'laagste peilgebied zonder gemaal'
  , isusable       = FALSE
WHERE
    type = 1
...
UPDATE
    tmp.peilgebieden_neighbours_1m
SET opmerking_peil = 'hoogste peilgebied zonder stuw of duiker'
  , isusable       = FALSE
WHERE
    type = 1
...
CREATE INDEX checks_fdla_sp_nowayout
ON
    checks.fdla_sp_nowayout
USING gist
    (
        geom
    )
