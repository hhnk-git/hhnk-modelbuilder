...
INSERT INTO nxt.polder(id, created, code, name, polder_type, geometry)
SELECT NEXTVAL('serial'), NOW(), code, 
...
FROM tmp.afvoergebiedaanvoergebied;
...
delete from nxt.fixeddrainagelevelarea;
...
DROP SEQUENCE IF EXISTS serial;
CREATE SEQUENCE serial START 1;  
INSERT INTO nxt.fixeddrainagelevelarea(id, created, code, name, type, peil_id, water_level_fixed, geometry)
SELECT nextval('serial'), -- door slechte geometry van peilgebieden (peilgebieden en afwijking liggen niet altijd netjes op elkaar..) gebruiken we hier een serial 
NOW(), -- created NOT NULL (dus timestamp opgeven)
code, 
case when (naam = ' ') IS NOT FALSE then 'LEEG' ELSE naam END, -- als veld damo.naam leeg is dan vullen we in 'LEEG' in nxt.name want nxt.name NOT NULL
case 
    when type like 'peilgebied' then 1 -- type (1 = peilvak, 2 = peilafwijking)
    when type like 'peilafwijking' then 2
end,
id, -- peil_id
wsa_level,
...
)),3)::geometry(MultiPolygonZ)
--) -- want nxt.geometry is 3D (met z-coordinaat)
FROM tmp.fixedleveldrainagearea_union;
...
UPDATE nxt.fixeddrainagelevelarea a SET water_level_summer = b.zomerpeil FROM hdb.hydro_deelgebieden b
WHERE a.code = 'HDB:' || b.code;

UPDATE nxt.fixeddrainagelevelarea a SET water_level_winter = b.winterpeil FROM hdb.hydro_deelgebieden b
WHERE a.code = 'HDB:' || b.code;
...
insert into nxt.channelsurface(id, created, code, geometry)
select objectid,
now(),
code,
...
DROP SEQUENCE IF EXISTS serial;
CREATE SEQUENCE serial START 1; 
delete from nxt.pumpstation;
insert into nxt.pumpstation(id, code, created, channel_type_id, type, name, num_timeseries, capacity, geometry)
select objectid,
...
case 
    ...
    when functiegemaal = 2 then 2 -- afvoergemaal wordt afvoergemaal
    when functiegemaal = 3 then 3 -- opmaling wordt opmaling
    when functiegemaal = 4 then 4 -- onderbemaling wordt onderbemaling
    when functiegemaal = 5 then 5 -- af- en aanvoergemaal wordt af- en aanvoergemaal
    when functiegemaal = 6 then 6 -- noodpomp wordt noodpomp
    when functiegemaal = 901 then 99 -- onderbemalingspomp-aanwezig-maar niet meer in gebruik wordt overig
    when functiegemaal = 902 then 99 -- trekkerpomp-niet standaard aanwezig wordt overig
    when functiegemaal = 903 then 99 -- op- en onderbemaling wordt overig
	when functiegemaal = 904 then 98 -- doorvoergemaal
    else 9999 -- de rest (dus ook 99 onbekend) wordt 9999
end, 
case when (naam = ' ') IS NOT FALSE then 'LEEG' ELSE naam END,
'9999' as num_timeseries,
maximalecapaciteit,  -- m3/uur naar m3/min
...
delete from nxt.weir;
insert into nxt.weir(id, created, code, type, type_function, channel_type_id, crest_width, crest_level, name, geometry, num_timeseries, shape)
select objectid,
now(),
case when (code = ' ') IS NOT FALSE then 'LEEG' ELSE code END, -- code
case when soortregelbaarheid is null then 9999 else soortregelbaarheid end, -- type
case when ws_functiestuw is null then 9999 else ws_functiestuw end, -- type_function
case 
    when ws_categorie is null then 9999     
    when ws_categorie > 4 then 9999 else ws_categorie -- channel_type_id = 1 primair, 2 secundair, 3 tertiair
end,
case 
    when kruinbreedte is not null and doorstroombreedte is not null and kruinbreedte > doorstroombreedte then doorstroombreedte
    when kruinbreedte is not null and doorstroombreedte is not null and kruinbreedte < doorstroombreedte then kruinbreedte
    when doorstroombreedte is not null then doorstroombreedte
    when kruinbreedte is not null then kruinbreedte
end,
...
'9999' as num_timeseries,
case 
    when ws_kruinvorm = 1 then 1 -- rechthoek wordt rectangle
    when ws_kruinvorm = 2 then 5 -- driehoek wordt tabulated trapezium
    when ws_kruinvorm = 3 then 5 -- trapezium wordt tabulated trapezium
    when ws_kruinvorm = 4 then 3 -- parabool wordt egg
    when ws_kruinvorm = 5 then 2 -- cirkel wordt cirkel
    when ws_kruinvorm = 6 then 4 -- kruin met speciale vorm wordt tabulated rectangle      
    when ws_kruinvorm = 99 then 9999 -- onbekend wordt leeg veld
    when ws_kruinvorm is null then 9999 -- leeg veld wordt leeg veld 
    else 9999
end from damo_ruw.stuw;
...
select setval('serial', max(id)) from nxt.weir;

insert into nxt.weir(id, created, code, type, type_function, channel_type_id, name, geometry, num_timeseries)
select nextval('serial'),
now(),
case when (code = ' ') IS NOT FALSE then 'LEEG' ELSE code END, -- code
5, -- type 
9999, -- type_fuctnion =9999 want niet aan-, af- of terugloopvoorziening
case 
    when ws_categorie is null then 9999     
    when ws_categorie > 4 then 9999 else ws_categorie -- channel_type_id = 1 primair, 2 secundair, 3 tertiair
end,
case when (naam = ' ') IS NOT FALSE then 'LEEG' ELSE naam END, -- name
st_force3d(st_transform(wkb_geometry,4326))::geometry(PointZ), -- van Point naar PointZ ??
'9999' as num_timeseries
...
ws_afsluitwijze2,
(st_dump(st_collect(st_transform(wkb_geometry, 4326)))).geom AS geom
from damo_ruw.duikersifonhevel group by objectid;

delete from nxt.culvert;
...
alter table nxt.culvert alter column type type integer using type::integer;
alter table nxt.culvert add column level_seperator_indicator boolean;

insert into nxt.culvert(id, created, code, channel_type_id, type_art, type, level_seperator_indicator, bed_level_upstream, bed_level_downstream, width, height, length, material, shape, num_timeseries, geometry)
select objectid,
now(),
code, -- code
case -- channel_type_id = 1 primair, 2 secundair, 3 tertiair
    when ws_categorie is null then 9999 
    when ws_categorie > 4 then 9999 else ws_categorie
end,
case  -- type_art = duiker (omdat typekruising = 1 Aquaduct, 2 Brug, 3 Duiker, 4 Sifon, 5 Hevel, 6 Bypass)
    when typekruising = 3 then 1 -- damo_ruw.duikersifonhevel.typekruising=duiker wordt nxt.culvert.type_art=duiker (type_art =1)
    when typekruising = 4 then 2 -- damo_ruw.duikersifonhevel.typekruising=sifon wordt nxt.culvert.type_art=sifon (type_art =2)
end,    
case -- type
    when typekruising = 3 and ws_inlaatfunctie = 'j' and (ws_afsluitwijze1 = 97 and (ws_afsluitwijze2 = 97 or ws_afsluitwijze2 is null)) then 1 -- 113 niet afsluitbare inlaat duikers
    when typekruising = 3 and ws_inlaatfunctie = 'n' and (ws_afsluitwijze1 = 97 and (ws_afsluitwijze2 = 97 or ws_afsluitwijze2 is null)) then 2 -- 13170 niet afsluitbare duikers (anders dan inlaat duiker)
	...
	when typekruising = 3 and ws_inlaatfunctie = 'j' and (ws_afsluitwijze1 is not null or ws_afsluitwijze2 is not null) then 3 -- 1479 afsluitbare inlaat duikers 
    when  typekruising = 3 and ws_inlaatfunctie = 'n' and (ws_afsluitwijze1 is not null or ws_afsluitwijze2 is not null) then 4 -- 14590 afsluitbare duikers (anders dan inlaat duiker)
    when typekruising = 4 and ws_inlaatfunctie = 'j' and (ws_afsluitwijze1 = 97 and (ws_afsluitwijze2 = 97 or ws_afsluitwijze2 is null)) then 5 -- 0 niet afsluitbare inlaat sifons = logisch
    when typekruising = 4 and ws_inlaatfunctie = 'n' and (ws_afsluitwijze1 = 97 and (ws_afsluitwijze2 = 97 or ws_afsluitwijze2 is null)) then 6 -- 0 niet afsluitbare sifons (anders dan inlaat sifon) = logisch
    when typekruising = 4 and ws_inlaatfunctie = 'j' and ((ws_afsluitwijze1 is not null and ws_afsluitwijze1 != 99) or (ws_afsluitwijze2 is not null and ws_afsluitwijze2 != 99)) then 7 -- 4 afsluitbare inlaat sifons = ??? zijn er inlaat sifons?
    when  typekruising = 4 and ws_inlaatfunctie = 'n' and ((ws_afsluitwijze1 is not null and ws_afsluitwijze1 != 99) or (ws_afsluitwijze2 is not null and ws_afsluitwijze2 != 99)) then 8 -- 189 afsluitbare sifons (anders dan inlaat sifon)
    else 9999
end,
...
lengte,
case 
    when soortmateriaal is null then 9999 -- leeg veld wordt leeg veld 
    when soortmateriaal = 1 then 9 -- 1 aluminium wordt 9 Staal
    when soortmateriaal = 2 then 5 -- 2 asbestcement wordt 5 Metselwerk
    when soortmateriaal = 3 then 1 -- 3 beton wordt 1	Beton
    when soortmateriaal = 4 then 9 -- 4 gegolfd plaatstaal	wordt 9	Staal
    when soortmateriaal = 5 then 5 -- 5 gewapend beton	wordt 5	Metselwerk
    when soortmateriaal = 6 then 4 -- 6 gietijzer	wordt 4	Gietijzer    
    when soortmateriaal = 7 then 9 -- 7 glad staal	wordt 9	Staal
    when soortmateriaal = 8 then 6 -- 8 glas wordt 6 HPE
    when soortmateriaal = 9 then 5 -- 9 grasbetontegels wordt 5 Metselwerk
    when soortmateriaal = 10 then 7 -- 10 hout	wordt 7	LDPE
    when soortmateriaal = 11 then 8 -- 11 ijser wordt 8 Plaatijzer
    when soortmateriaal = 12 then 4 -- 12 koper wordt 4 Gietijzer
    when soortmateriaal = 13 then 2 -- 13 kunststof wordt 2 Pvc
    when soortmateriaal = 14 then 2 -- 14 kuststoffolie wordt 2 Pvc
    when soortmateriaal = 15 then 5 -- 15 kurk	5 wordt Metselwerk
    when soortmateriaal = 16 then 4 -- 16 lood	4 wordt Gietijzer
    when soortmateriaal = 17 then 5 -- 17 metselwerk wordt 5 Metselwerk
    when soortmateriaal = 18 then 9 -- 18 plaatstaal wordt	9 Staal  
    when soortmateriaal = 19 then 5 -- 19 puinsteen wordt 5 Metselwerk
    when soortmateriaal = 20 then 2 -- 20 PVC wordt 2 Pvc
    when soortmateriaal = 21 then 9 -- 21 staal wordt 9 Staal
    when soortmateriaal = 22 then 1 -- 22 steen wordt 1 Beton
    when soortmateriaal = 23 then 1 -- 23 voorgespannen beton wordt 1 Beton
    when soortmateriaal = 98 then 9999 -- onbekend wordt leeg veld
    when soortmateriaal = 99 then 9999 -- overig wordt leeg veld
    else 9999
end,
case
    when vormkoker is null then 9999 -- leeg wordt leeg
    when vormkoker = 1 then 2 -- rond wordt circle
    when vormkoker = 2 then 1 -- driehoekig wordt rectangle
    when vormkoker = 3 then 1 -- rechthoekig wordt rectangle
    when vormkoker = 4 then 3 -- eivormig wordt egg
    when vormkoker = 5 then 5 -- ellipsvormig wordt ellips
    when vormkoker = 6 then 4 -- paraboolvormig wordt muil
    when vormkoker = 7 then 6 -- trapeziumvormig wort heul
    when vormkoker = 8 then 6 -- heulprofiel wordt heul
    when vormkoker = 9 then 4 -- muilprofiel wordt muil
    when vormkoker = 10 then 1 -- langwerpig wordt rectangle
    when vormkoker = 11 then 1 -- scherp wordt rectangle
    when vormkoker = 99 then 9 -- onbekend wordt onbekend
    else 9999
    end,
'9999' as num_timeseries,
st_force3d(st_transform(geom,4326))::geometry(LineStringZ) -- van (wkb_geometry, 4326) naar geometry geometry(LineStringZ,4326),
from tmp.duikersifonhevel_sp where (typekruising = 3 or typekruising = 4) AND code NOT IN ('KDU-P-36517','KDU-M-4872'); -- damo_ruw.duikersifonhevel.typekruising = 1 Aquaduct, 2 Brug, 3 Duiker, 4 Sifon, 5 Hevel, 6 Bypass
...
SELECT setval('seq_nxt_culvert', (SELECT max(id) FROM nxt.culvert));

INSERT INTO nxt.culvert(id, created, code, channel_type_id, type_art, type, bed_level_upstream, bed_level_downstream, width, height, length, material, shape, num_timeseries, geometry)
SELECT
	...
	CASE -- channel_type_id = 1 primair, 2 secundair, 3 tertiair
		...
		ELSE ws_categorie
	END,
	2, --Aquaduct als syfon nxt.culvert.type = 2
	6, --niet afsluitbare syfon (geen inlaat)
...
delete from nxt.bridge;
insert into nxt.bridge(id, created, code, name, type, width, bottom_level, channel_type_id, geometry)
select distinct on (brug_id) brug_id, -- distinct want er komt eenmaal een dubbele in voor (brugid 213333)
now(),
code, -- code wordt code
code, -- code wordt name want name NOT NULL
'null', -- type
null, --> widht vullen we met sum(doorstroombreedte) --> dat gebeurt hieronder
hoogteonderzijde, --bottom_level (mNAP)
case
    when ws_categorie is null then 9999 
    when ws_categorie > 4 then 9999 else ws_categorie -- channel_type_id = 1 primair, 2 secundair, 3 tertiair
end,
...
create table tmp.bridge as select distinct on (brug_id) * from damo_ruw.brug;
...
with conversie as (select brugid, count(*) from damo_ruw.doorstroomopening group by brugid)
update tmp.bridge a set number_openings = count from conversie b where a.brug_id = b.brugid; 
update tmp.bridge a set total_flow_width = (select sum(b.breedteopening) from damo_ruw.doorstroomopening b where a.brug_id = b.brugid);
update nxt.bridge a set number_openings = b.number_openings from tmp.bridge b where a.id = b.brug_id;
update nxt.bridge a set width = b.total_flow_width from tmp.bridge b where a.id = b.brug_id;
...
create table nxt.fixed_dam as select
objectid::integer as id,
code::varchar,
case  -- channel_type_id = 1 primair, 2 secundair, 3 tertiair
    when ws_categorie is null then 9999 
    when ws_categorie > 4 then 9999 else ws_categorie::integer
...
delete from nxt.crosssection;
drop sequence if exists serial;
create sequence serial start 1;
insert into nxt.crosssection(id, created, cross_profile_id, channel_id, prw_id, bed_level, bank_level, width, geometry)
select nextval('serial'), now(), pro_id, channel_id, prw_id, min, max, max_width, st_force3d(st_transform(geom,4326))::geometry(PointZ) from tmp.tabulated_table;
...
delete from nxt.crossprofile;
insert into nxt.crossprofile(id, prw_id, created, type, width, height)
select pro_id,
prw_id,
now(),
5, -- type 1: rectangle = 1, circle = 2, egg = 3, tabulated_rectangle = 5, tabulated_trapezium = 6 (overgenomen van 3di versie2)
width, height from tmp.tabulated_table;
...
insert into nxt.sluice(id, created, name, code, channel_type_id, type, width, length, bottom_level, geometry)
select objectid, now(), 
case when (naam = ' ') IS NOT FALSE then 'LEEG' ELSE naam END, -- name
case when (code = ' ') IS NOT FALSE then 'LEEG' ELSE code END, -- code
case  -- channel_type_id = 1 primair, 2 secundair, 3 tertiair
...
case -- type
    when ws_inlaatfunctie = 'j' and (ws_afsluitwijze1 = 97 and (ws_afsluitwijze2 = 97 or ws_afsluitwijze2 is null)) then 1 -- niet afsluitbare inlaat sluis
    when ws_inlaatfunctie = 'n' and (ws_afsluitwijze1 = 97 and (ws_afsluitwijze2 = 97 or ws_afsluitwijze2 is null)) then 2 -- niet afsluitbare sluis (anders dan inlaat sluis)
    when ws_inlaatfunctie = 'j' and ((ws_afsluitwijze1 is not null and ws_afsluitwijze1 != 99) or (ws_afsluitwijze2 is not null and ws_afsluitwijze2 != 99)) then 3 -- afsluitbare inlaat sluis
    when ws_inlaatfunctie = 'n' and ((ws_afsluitwijze1 is not null and ws_afsluitwijze1 != 99) or (ws_afsluitwijze2 is not null and ws_afsluitwijze2 != 99)) then 4 -- afsluitbare sluis(anders dan inlaat sluis)
    else 9999
end,
case 
    when breedte is not null and doorvaartbreedte is not null then least(breedte, doorvaartbreedte) 
    when breedte is not null and doorvaartbreedte is null then breedte
    when breedte is null and doorvaartbreedte is not null then doorvaartbreedte
end,
ws_doorvaarlengte,
case
    when hoogtebinnenonderkantben is not null and hoogtebinnenonderkantbov is not null and drempelhoogte is not null then greatest(hoogtebinnenonderkantben, hoogtebinnenonderkantbov, drempelhoogte) 
    when hoogtebinnenonderkantben is not null and drempelhoogte is not null then greatest(hoogtebinnenonderkantben, drempelhoogte)
    when hoogtebinnenonderkantbov is not null and drempelhoogte is not null then greatest(hoogtebinnenonderkantbov, drempelhoogte)
    when drempelhoogte is not null then drempelhoogte
    when hoogtebinnenonderkantben is not null and hoogtebinnenonderkantbov is null then hoogtebinnenonderkantben
    when hoogtebinnenonderkantben is null and hoogtebinnenonderkantbov is not null then hoogtebinnenonderkantbov
end,
st_force3d(st_transform(wkb_geometry,4326))::geometry(PointZ) -- van Point naar PointZ ??
from damo_ruw.sluis;
...
-- Vragen
-- Zijn er voor velden zoals type als codes in gebruik?
-- Hoe werkt koppeling pump en pumpstation?
-- Visspassage als aparte tabel of als type stuw? --> type stuw
-- Sluis als nieuwe tabel of als type stuw? sluis nu niet in lizard
-- Sifon als type duiker of aparte tabel? --> type duiker


-- nxt.blabla id serial NOT NULL --> hier kunnen we niet code van DAMO nemen want code is meestal string (met letters) en iig niet een serial. Dus objectid van DAMO nemen als nxt.blabla id serial NOT NULL ?? 
-- Wouter: "ja, serial middels nextval is prima voor nxt.id en dan voor nxt.code nemen we de damo.code (string)"

-- nxt.hydra_core_polder --> geometry geometry(MultiPolygonZ,4326),
-- dus van damo_ruw naar nxt.hydro_core_polder moet ik een st_force3d doen (z-coordinaat van 0?? toevoegen) want anders nxt.polder.geometry has Z dimension but damo_ruw.geometry does not
-- Wouter: "bespreken met Ber.. later weer omzetten naar 2d punt voor 3di??? waarom niet column type in nxt aanpassen??
