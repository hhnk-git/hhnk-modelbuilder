...
-- eerst maken we die dwarslijnen single part ipv MultiLineStringZ
...
from damo_ruw.gw_pro group by pro_id; */
drop sequence if exists serial;
create sequence serial start 1;
drop table if exists tmp.gw_pro_sp;
create table tmp.gw_pro_sp as
...
select * from damo_ruw.gw_pbp 
where prw_prw_id in (select prw_id from damo_ruw.gw_prw where osmomsch = 'Z2') 
AND pbpsoort = ANY (ARRAY[5,6,7,22]); 
--Array lijst met 'natte' metingen, evt uitbreiden adhv lijst Matthijs
-- 470460 YZ punten (hetzelfde als like 'Z1')

drop table if exists tmp.yz_table;
create table tmp.yz_table as 
select prw_prw_id::varchar as name, prw_prw_id::integer, round(iws_hoogte::numeric,3) as z, round(iws_afstand::numeric,3) as y 
from tmp.profielpunten_selectie order by prw_prw_id, iws_afstand; 
-- deze order is belangrijk!! niet weghalen!!
...
RETURNS setof tabulated_type AS $$
	DECLARE 
		...
		max_width float;
	BEGIN
		min = min(unnest) from (select unnest(z)) b;
		max = max(unnest) from (select unnest(z)) b;
		length = array_length(z, 1);
		indicis_min = array_positions(z, min);
		index_first_min = min(unnest) from (select unnest(indicis_min)) foo;
		index_last_min = max(unnest) from (select unnest(indicis_min)) foo;
		max_left = max(unnest) from (select unnest(z[1:index_first_min])) b;
		max_right = max(unnest) from (select unnest(z[index_last_min:length])) b;
		index_left_max = max(unnest) from (select unnest(array_positions(z[1:index_first_min], max_left))) foo;
		index_right_max = index_last_min-1 + min(unnest) from (select unnest(array_positions(z[index_last_min:length], max_right))) foo;
	...
	LOOP	
        ...
        END LOOP;
        max_height = max(unnest) from (select unnest(height)) foo;
        max_width =  max(unnest) from (select unnest(width)) foo;
        return query
	select height, width, max_height, max_width, min, max, length;
END; $$
LANGUAGE PLPGSQL;   
...
create table tmp.tabulated_table as select 
	name as defname, 
	...
	FROM (SELECT * FROM tmp.yz_table) foo
	group by name, prw_prw_id;

-- select * from tmp.tabulated_table
-- 16.524 profielen (arrays van yz waarden)
...
update tmp.tabulated_table a 
set pro_id = b.pro_pro_id
from damo_ruw.gw_prw b where a.prw_id = b.prw_id;

-- nxt.channel maken en vullen vanuit damo (ja,,, dit gebeurt ook op productie, maar 2x is handiger anders tussendoor met ogr2ogr klootvioolen);
...
ws_talud_rechts,
ws_bodembreedte,
breedte_getabuleerd as tabulated_width, --derived_width_at_waterlevel,
hoogte_getabuleerd as tabulated_height, --derived_bed_level,
bodemhoogte_nap as derived_bed_level, --verander bodemhoogte in nieuw aangeleverde kolom.
...
-- volgens Jeroen B niet alleen koppelen obv geometrie maar ook obv van ovk_ovk_id en daarvoor moeten we t originele objectid van het hydroobject hebben. Dit originele objectid gaat verloren tijdens hun damo export. Daarom sturen ze nog een code_hydroobject_objectid.dbf mee)

/*
code
select code from tmp.hydroobject_sp limit 500;
"OAF-S-325"
"OAF-CS-24060"
"OAF-AA-556"

select hydroobj_2 from damo_ruw.code_hydroobject_objectid;
"OAF-S-325"
"OAF-CS-24060"
"OAF-AA-556"
*/
...
update tmp.hydroobject_sp a set objectid_ori = b.objectid 
from fixed_data.code_hydroobject_objectid b where a.code = b.hydroobj_2;
...
insert into nxt.channel(id, objectid_ori, created, code, channel_type_id, type, name, bed_level, talud_left, talud_right, bed_width, tabulated_width, tabulated_height, derived_bed_level, geometry)
select serial,
objectid_ori,
now(),
...
(st_dump(st_collect(st_transform(st_force2d(geometry), 28992)))).geom::geometry(Linestring,28992) AS geom from nxt.channel group by id;

drop index if exists tmp.gw_pro_sp_index;
create index gw_pro_sp_index on tmp.gw_pro_sp using gist(geom);
cluster tmp.gw_pro_sp using gw_pro_sp_index;

drop index if exists tmp.channel_28992_sp_index;
create index channel_28992_sp_index on tmp.channel_28992_sp using gist(geom);
cluster tmp.channel_28992_sp using channel_28992_sp_index; 

update tmp.tabulated_table a
set channel_id = b.nxt_channel_id from tmp.channel_28992_sp b, tmp.gw_pro_sp c
where a.pro_id = c.pro_id and st_crosses(b.geom, c.geom);
...
from tmp.channel_28992_sp b, tmp.gw_pro_sp c
where a.channel_id is not null 
and a.channel_id = b.nxt_channel_id
and a.pro_id = c.pro_id; -- 21 sec

alter table tmp.tabulated_table add column ovk_id integer;
update tmp.tabulated_table a set ovk_id = b.ovk_ovk_id
from tmp.gw_pro_sp b where a.pro_id = b.pro_id;

update tmp.tabulated_table a set channel_id = b.nxt_channel_id,
opmerking = 'koppel obv ovk_id en hydrobjectid_ori'
from tmp.channel_28992_sp b
where a.ovk_id = b.objectid_ori and a.channel_id is null and a.ovk_id is not null;

update tmp.tabulated_table a set 
numGeometries = 1,
geom = ST_LineInterpolatePoint(b.geom, 0.5),
opmerking = '>1 pro_ids per hydroobject obv code wordt 1 profielpunt' 
from tmp.channel_28992_sp b 
where a.ovk_id = b.objectid_ori
and a.pro_id in (select distinct on (channel_id) pro_id from tmp.tabulated_table where opmerking like 'koppel obv ovk_id en hydrobjectid_ori');

update tmp.tabulated_table set
opmerking = 'dit hydroobject heeft al een profiel obv code'
where geom is null
and opmerking like 'koppel obv ovk_id en hydrobjectid_ori';

update tmp.tabulated_table set
opmerking = 'koppel niet mogelijk obv geom of code'
where opmerking is null;
...
create table tmp.tabulated_table_backup as select * from tmp.tabulated_table;

--select distinct on (opmerking) opmerking from tmp.tabulated_table_backup order by opmerking;
--">1 pro_ids per hydroobject obv code wordt 1 profielpunt"
--"dit hydroobject heeft al een profiel obv code"
--"koppel niet mogelijk obv geom of code"
--"koppel obv geom"

--select distinct on (pro_id) * from tmp.tabulated_table_backup;
-- 38.479 waarvan (38.479 uniek pro_id)
--select distinct on (geom) * from tmp.tabulated_table_backup;
-- 38.479 waarvan (37.214 unieke geom)

-- de niet duplicate

drop table if exists tmp.tabulated_table_backup_not_duplicate;
create table tmp.tabulated_table_backup_not_duplicate as select distinct on (geom) * from tmp.tabulated_table_backup;
--select * from tmp.tabulated_table_backup_not_duplicate;
-- 37.213

-- de overgebleven selecteren als duplicate
drop table if exists tmp.tabulated_table_backup_duplicate;
create table tmp.tabulated_table_backup_duplicate as select * from tmp.tabulated_table_backup where pro_id not in (select pro_id from tmp.tabulated_table_backup_not_duplicate);
--select * from tmp.tabulated_table_backup_duplicate;
-- 1.266

drop table if exists tmp.tabulated_table_backup;
create table tmp.tabulated_table_backup as select * from tmp.tabulated_table_backup_not_duplicate;
-- 37.214
-- select distinct on (opmerking) opmerking from tmp.tabulated_table_backup order by opmerking;
--">1 pro_ids per hydroobject obv code wordt 1 profielpunt"
--"koppel niet mogelijk obv geom of code"
--"koppel obv geom"
update tmp.tabulated_table_backup set opmerking = concat_ws(',','not_duplicate',opmerking);
-- select distinct on (opmerking) opmerking from tmp.tabulated_table_backup order by opmerking;
--"not_duplicate,>1 pro_ids per hydroobject obv code wordt 1 profielpunt"
--"not_duplicate,koppel niet mogelijk obv geom of code"
--"not_duplicate,koppel obv geom"
insert into tmp.tabulated_table_backup(defname, prw_id, width, height, max_height, max_width, min, max, nr_input_points, channel_id, pro_id, geom, numgeometries, opmerking, ovk_id)
select defname, prw_id, width, height, max_height, max_width, min, max, nr_input_points, channel_id, pro_id, geom, numgeometries, concat_ws(',','duplicate',opmerking), ovk_id from tmp.tabulated_table_backup_duplicate;
--select distinct on (opmerking) opmerking from tmp.tabulated_table_backup order by opmerking;

--"duplicate,dit hydroobject heeft al een profiel obv code" 			geen geom -- 41
--"duplicate,koppel niet mogelijk obv geom of code"				geen geom -- 191
--"duplicate,koppel obv geom" 							wel geom -- 1034
--"not_duplicate,>1 pro_ids per hydroobject obv code wordt 1 profielpunt"	wel geom -- 294					
--"not_duplicate,dit hydroobject heeft al een profiel obv code"			geen geom -- 1		
--"not_duplicate,koppel obv geom"						wel geom -- 36918

update tmp.tabulated_table_backup set opmerking = 'dit hydroobject heeft al een profiel obv code' where opmerking = 'duplicate,dit hydroobject heeft al een profiel obv code';
update tmp.tabulated_table_backup set opmerking = 'koppel niet mogelijk obv geom of code' where opmerking = 'duplicate,koppel niet mogelijk obv geom of code';
update tmp.tabulated_table_backup set opmerking = 'dit hydroobject heeft al een profiel obv code' where opmerking = 'not_duplicate,dit hydroobject heeft al een profiel obv code';
--"dit hydroobject heeft al een profiel obv code"
--"duplicate,koppel obv geom"
--"koppel niet mogelijk obv geom of code"
--"not_duplicate,>1 pro_ids per hydroobject obv code wordt 1 profielpunt"
--"not_duplicate,koppel obv geom"
...
/*
select * from tmp.tabulated_table_backup where opmerking like 'duplicate,dit hydroobject heeft al een profiel obv code'
-- 42 (geom is null)
select * from tmp.tabulated_table_backup where opmerking like 'duplicate,koppel niet mogelijk obv geom of code'
-- 190 (geom is null)
select * from tmp.tabulated_table_backup where opmerking like 'duplicate,koppel obv geom'
-- 1033 (geom is not null)
select * from tmp.tabulated_table_backup where opmerking like 'not_duplicate, >1 pro_ids per hydroobject obv code = gekozen'
-- 294 (geom is not null
select * from tmp.tabulated_table_backup where opmerking like 'not_duplicate,koppel niet mogelijk obv geom of code'
-- 1 (geom is null)
select * from tmp.tabulated_table_backup where opmerking like 'not_duplicate,koppel obv geom'
-- 36919 (geom is not null)
--select * from tmp.tabulated_table_backup where geom is null 
-- 233 (geom is null)
*/

drop table if exists tmp.tabulated_table;
create table tmp.tabulated_table as select * from tmp.tabulated_table_backup;
drop table if exists tmp.tabulated_table_backup;
drop table if exists tmp.tabulated_table_backup_not_duplicate;
drop table if exists tmp.tabulated_table_backup_duplicate;
