-- kanalen worden voorbereid (nxt >> checks): er wordt een vereenvoudigd kanalen netwerk aangemaakt
-- output = checks.channel en checks.channel_linemerge 

-- copy channels to checks schema
DROP TABLE IF EXISTS checks.channel;
CREATE TABLE checks.channel AS
SELECT id, organisation_id, created, code, type, bed_level, comment, 
       name, talud_left, talud_right, image_url, 
       ...
       "end", start, 
       channel_type_id, bed_width, tabulated_width, tabulated_height, derived_bed_level
  FROM nxt.channel
--LIMIT 10000
;
CREATE INDEX checks_channel_geom ON checks.channel USING gist(geom);
CREATE INDEX checks_channel_bufgeom ON checks.channel USING gist(bufgeom);
CREATE INDEX checks_channel_pointgeom ON checks.channel USING gist(pointgeom);

-- welke kanalen zijn een sifon?
UPDATE checks.channel as a
SET comment = 'sifon'
FROM checks.culvert as b
WHERE b.type_art = 2 AND ST_Contains(a.bufgeom,b.geom)
;
...
-- 0) sidestep van 'welke kanalen zijn een sifon?':
-- er zijn waarschijnlijk sifons die wel intersecten met een kanaal maar (net) niet helemaal de kanaalbuffer vallen. Deze sifons krijgen een opmerking '>51% in 1 hydroobject'. Door te filteren op deze opmerking kan hhnk voorselecteren tijdens het oplossen van sifons die niet in een checks.channel containen.
...
SELECT distinct on (a.id) a.*, b.id as culvert_id 
    FROM checks.channel a, checks.culvert as b
...
CREATE INDEX tmp_channel_sinfon_geom ON tmp.channel_sifon_snap USING gist(geom);
CREATE INDEX tmp_channel_sifon_bufgeom ON tmp.channel_sifon_snap USING gist(bufgeom);
CREATE INDEX tmp_channel_sifon_pointgeom ON tmp.channel_sifon_snap USING gist(pointgeom);
...
CREATE TABLE tmp.sifon AS SELECT * FROM checks.culvert WHERE type_art=2;

-- 1.2) sifon bufferen
ALTER TABLE tmp.sifon ADD COLUMN bufgeom geometry(Polygon,28992);
UPDATE tmp.sifon SET bufgeom = st_buffer(geom,0.1);

-- 1.3) overlapping (percentage) bepalen
UPDATE tmp.channel_sifon_snap a 
	SET sifon_share = 
	round(
		(ST_area(ST_intersection(a.bufgeom, b.bufgeom))/(ST_area(b.bufgeom))*100)::numeric,2)
	FROM tmp.sifon b
...
WITH list as (
	SELECT count(b.*), a.id as channel_id 
	FROM tmp.channel_sifon_snap a, tmp.sifon b
	WHERE ST_Intersects(a.bufgeom, b.geom)
	GROUP BY a.id)
UPDATE tmp.channel_sifon_snap c SET number_of_sifons = d.count from list d where c.id = d.channel_id;
...
UPDATE checks.culvert a SET opmerking = concat_ws(',',opmerking,'>51% in 1 hydroobject') where a.id in (select culvert_id from tmp.channel_sifon_snap where sifon_share > 51 and number_of_sifons<2);
...
CREATE TABLE checks.channel_linemerge AS SELECT (
...
SELECT NULL, -- lijngeometrie niet meegenomen zodat stuwen op uiteinde van sifon niet als 2 watergangen gemarkeerd worden
...
WHERE comment = 'sifon'
;
CREATE INDEX checks_channel_linemerge_geom ON checks.channel_linemerge USING gist(geom);
CREATE INDEX checks_channel_linemerge_bufgeom ON checks.channel_linemerge USING gist(bufgeom);

-- voeg id toe
ALTER TABLE checks.channel_linemerge ADD COLUMN id integer;
DROP SEQUENCE IF EXISTS serial; 
CREATE SEQUENCE serial;
UPDATE checks.channel_linemerge SET id = nextval('serial');
...
UPDATE checks.channel_linemerge a
SET channel_type_id = b.channel_type_id
FROM checks.channel as b
WHERE ST_Contains(a.bufgeom,b.pointgeom)
;